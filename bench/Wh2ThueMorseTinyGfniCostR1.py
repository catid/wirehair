#!/usr/bin/env python3
"""All-sample .75 completion-relative full-encoder screen; replay never executes a codec."""
import argparse
...
ROOT = A.ROOT
PROTOCOL = "wirehair.wh2.thue-tiny-gfni-cost-r1"
OUTPUT = Path("/var/tmp/wh2-thue-tiny-gfni-cost-r1")
TARGET = "wh2_thue_tiny_gfni_cost_r1"
RAW_CAP, ERR_CAP, TOTAL_CAP = 32 * 1024**2, 65536, 64 * 1024**2
WORKER_WALL, WORKER_CPU, CONTROLLER_WALL, OBSERVER_WALL = 45, 40, 60, 70
WORK_CAP = 30 * 10**9
BATCH = 128
...
CALLBACKS = 12 * 2 * 3 * 7 * 18
NEW_SOURCES = tuple("bench/" + name for name in (
    "Wh2ThueMorseTinyGfniCostR1.py", "test_Wh2ThueMorseTinyGfniCostR1.py",
    "Wh2ThueMorseTinyGfniCostR1.cpp",
    "Wh2ThueMorseTinyGfniCostR1/CMakeLists.txt"))
...
def statistics(records):
    ...
    require(len(groups) == 42, "all separate width/metric/class/order groups")
    ...
    for (width, metric, comparison, order), values in sorted(groups.items()):
        ...
        if comparison < 4:
            ...
            # Wider payloads need equivalence, not an invented gain.
            # Every direct WH1 comparison must be a resolved win; no 5% floor.
            limit = 0.0 if comparison == 5 or (comparison == 4 and width == 0) else bound
...
def current_receipt(build, deadline):
    ...
    support = ("Wh2ThueMorseTinyGfniR0.cpp", "Wh2ThueMorseNativeCodec.cpp",
               ...
               "Wh2ThueMorseMatvecCostR0Bridge.cpp", "Wh2ThueMorseMatvecCostR0Bridge.cpp",
               "Wh2ThueMorseTinyGfniCostR1.cpp", "Wh2RdpruTargetIdentityV2.cpp",
