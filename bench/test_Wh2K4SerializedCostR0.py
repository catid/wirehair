...
class CostTest(unittest.TestCase):
    ...
    def test_invalid_partial_spool_bundle_replays(self):
        with tempfile.TemporaryDirectory(prefix='wh2-k4-cost-invalid-test-') as d:
    ...
    def test_required_frozen_sizes_and_ownership_pairs(self):
        ...
        self.assertEqual(C.ARM_NAMES,('ordinary_independent','k4_independent','wh1_owned',
                                     'ordinary_borrowed','k4_borrowed','wh1_borrowed'))
        self.assertEqual(len(C.selected_rows()),20)
    ...
    def test_old_scientific_namespace_and_work_unchanged(self):
        ...
        self.assertEqual(C.OUTPUT,Path('/var/tmp/wh2-k4-serialized-cost-r0'))
    ...
    def test_partial_tail_bytes_and_guards(self):
        for shape,(b,tail) in enumerate(C.SHAPES):
            ...
            self.assertEqual(f['packet_bytes'],[b]*3+[tail]+[b]*16)
            packets=bytes.fromhex(f['arms'][1]['packets'])
            self.assertEqual(packets[3*b+tail:4*b],bytes([0xa5])*(b-tail))
            if tail<b:
                changed=copy.deepcopy(self.header)
                changed['fixtures'][shape]['arms'][1]['packets']=(
                    packets[:3*b+tail]+b'\x00'+packets[3*b+tail+1:]).hex()
    ...
    def test_actual_feed_endpoint_and_fallback_are_charged(self):
        ...
        # Zero control repair rows require all four systematic fallback packets.
        ...
        for arm in (0,3):
            f['arms'][arm].update(rows=bytes(v for row in rows for v in row).hex(),
                packets=C.payload(rows,b,tail),steps=[12,12],fallback=[True,True])
        prior=copy.deepcopy(self.old);prior['fixtures']=copy.deepcopy(header['fixtures'])
        C.verify_fixtures(header,prior)
        records=copy.deepcopy(self.records)
        for record in records:
            c=record['coordinate']
            if c[3]==1 and c[4]>0 and c[7] in (0,3):record['counts'][3]=128*12
