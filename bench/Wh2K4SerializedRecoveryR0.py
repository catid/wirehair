#!/usr/bin/env python3
"""Frozen K4 serialized recovery; six ownership-matched arms, retained cohort only."""
import argparse
...
U = sibling('_k4_recovery_build_io', 'Wh2K4RecoveryBuildR0.py')
D = sibling('_k4_recovery_retained_data', 'Wh2K4NativeDataR0.py')
A = U.A
ROOT = U.ROOT
PROTOCOL = 'wirehair.wh2.k4-serialized-recovery-r0'
OUTPUT = Path('/var/tmp/wh2-k4-serialized-recovery-r0')
MODES = ('native', 'scalar', 'asan')
WIDTHS = (2, 64, 1280)
ARMS = ('ordinary_wh2_independent','k4_independent','wh1_owned',
        'ordinary_wh2_borrowed','k4_borrowed','wh1_borrowed')
K = 4
RAW_CAP, ERR_CAP = 96*1024**2, 65536
BUNDLE_CAP = 384*1024**2
...
SOURCES = ('bench/Wh2K4SerializedRecoveryR0.cpp', 'bench/Wh2SmallRecoveryWorkerR0.h',
           'bench/Wh2K4SerializedRecoveryR0.py', 'bench/test_Wh2K4SerializedRecoveryR0.py',
           'bench/Wh2K4RecoveryBuildR0.py', 'bench/test_Wh2K4RecoveryBuildR0.py',
           'bench/Wh2K4SerializedRecoveryR0.md', 'bench/Wh2K4NativeDataR0.py',
...
def roster(neutral=False):
    ...
    result = []
    if neutral:
        streams = [list(range(8)),list(range(4,12)),[0xffffffff-2*j for j in range(8)],[0]*8]
        for width in WIDTHS:
        ...
        A.exact(len(result),6254,'retained trace/origin roster')
        A.exact(sum(len(e['ids']) for e in result),49891,'exact original packet horizons')
    return result
...
def summarize(records,expected):
    ...
    def paired(selected,historical=False):
        ...
        for candidate,control in PAIRED:
            ...
            pairs.append(pair)
        return pairs
    def curves(selected,historical=False):
        return [[sum(r['arms'][a]['first']==0 or r['arms'][a]['first']>K+overhead
                     for r,e in selected if not historical or len(e['ids'])>=K+overhead)
    ...
    for group,name in enumerate(('retained_fresh','hard','historical_original_width')):
        ...
        groups.append(dict(group=name,cases=len(selected),
            failure_counts_oh0_to_4=curves(selected,group==2),
            eligible_cases_oh0_to_4=[sum(len(e['ids'])>=K+oh for _,e in selected) for oh in range(5)],
    ...
    for row,e in zip(records,expected):
        ...
        for arm,r in enumerate(row['arms']):
            ...
            if arm in (0,3):
                ...
                A.exact(profile,struct.pack('<4sHHQQII',b'WHK4',1,32,0x5748324b34544d31,size,width,0),'sealed WHK4 identity')
            ...
            for packet_id, coefficients in zip(ids, observed):
                key = (arm % 3, r['profile'], width, tail, packet_id)
                A.exact(coefficients, repeated_rows.setdefault(key, coefficients), 'repeated packet row identity')
            if arm in (1,4):
                A.exact(observed,tuple(coefficient(i) for i in ids),'independent sealed K4 rows')
            ...
            A.exact(r['counts'],[5,5*len(ids),1,first if first else len(ids),2 if first else 0,6],'whole attempted API ledger')
...
def current(receipt):
    ...
    for mode in MODES:
        ...
        artifacts = U.pin_map(manifest['artifacts'])
        fixture = str(executable.parent/'fixtures.jsonl')
        A.require(str(executable) in artifacts, 'built executable artifact')
        A.require(fixture in artifacts and fixture in declared, 'pinned neutral fixture artifact')
        A.exact(artifacts[fixture], declared[fixture], 'neutral fixture identity binding')
...
def capture(executable, claim, deadline, spools):
    ...
    try:
        ...
        child = subprocess.Popen([str(executable), '--worker', claim], stdin=subprocess.DEVNULL,
                                 ...
                                 env=U.process_environment(), cwd=ROOT)
...
def run(path):
    ...
    A.require(sum(p['bytes'] for p in members) < BUNDLE_CAP-65536, 'bundle cap')
...
def replay():
    ...
    A.require(complete['outcome'] in ('PASS','FAIL','INVALID'), 'sealed outcome enum')
    ...
    U.pin_map(complete['files'])
    A.require(sum(p['bytes'] for p in complete['files']) < BUNDLE_CAP-65536, 'bundle cap')
    A.require((OUTPUT/'COMPLETE.json').stat().st_size <= 65536, 'complete publication cap')
    for p in complete['files']:
        name = Path(p['path']).name
        cap = (RAW_CAP if name.endswith('.raw.jsonl') else ERR_CAP if name.endswith('.stderr.txt') else
               1024**2 if name == 'CLAIM.json' else 16*1024**2)
        A.require(p['bytes'] <= cap, 'sealed member cap')
        A.exact(U.pin(p['path']),p,'sealed member identity')
    ...
    A.require(type(elapsed) in (int,float) and math.isfinite(elapsed) and elapsed >= 0, 'finite nonnegative elapsed time')
    if complete['outcome']=='INVALID':
        A.exact(set(stored), {'protocol','outcome','execution_valid','failure','elapsed_seconds'}, 'INVALID analysis schema')
        A.exact(stored['execution_valid'], False, 'INVALID execution predicate')
        A.require(type(stored['failure']) is str, 'INVALID failure string')
    if complete['outcome']!='INVALID':
        reference = None
        for mode in MODES:
