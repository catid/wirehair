...
def synthetic():
    ...
    for b in C.WIDTHS:
        candidate_profile, candidate_packets = C.candidate_fixture(b)
        generic = dict(profile='00'*32, packets=candidate_packets, steps=[5,5], rows=rows)
        candidate = dict(profile=candidate_profile, packets=candidate_packets, steps=[5,5], rows=rows)
        certified = copy.deepcopy(generic)
        certified['profile'] = struct.pack('<4sHHQQII',b'WHV2',1,32,0x4b295bbb47f4f9c9,5*b,b,0).hex()
        arms = [certified,candidate,generic]
        fixtures.append(dict(width=b,source=bytes((37*i+i//11) % 256 for i in range(5*b)).hex(),
    ...
    for coordinate in C.roster():
        ...
        counts = [0 if metric else 128, 0 if metric else 2304, 128 if metric else 0,
                  640 if metric else 0, 128 if metric else 0, 128]
...
class CostTest(unittest.TestCase):
    ...
    def test_whole_synthetic_stream(self):
        ...
        self.assertEqual(len(result['statistics']), 180)
    ...
    def test_roster_independent_coverage(self):
        ...
        self.assertEqual(len(counts), 2160)
        self.assertEqual(set(counts.values()), {1})
        self.assertEqual(len(phases), 8640)
    ...
    def test_candidate_oracle_and_descriptor_are_required(self):
        ...
        header = copy.deepcopy(self.header)
        header['fixtures'][0]['arms'][1]['packets'] = '00'*36
        header['fixtures'][0]['arms'][4]['packets'] = '00'*36
        old = copy.deepcopy(self.old)
        old['fixtures'][0]['arms'][1]['packets'] = '00'*36
        old['fixtures'][0]['arms'][4]['packets'] = '00'*36
    ...
    def test_required_frozen_sizes_and_ownership_pairs(self):
        ...
        self.assertEqual(C.CALLBACKS,38880)
        self.assertEqual(C.ARM_NAMES,('certified_independent','k5_independent','wh1_owned',
                                     'certified_borrowed','k5_borrowed','wh1_borrowed'))
        self.assertEqual(len(C.selected_rows()),18)
    ...
    def test_public_descriptor_cannot_use_prototype_or_other_profile(self):
        for profile in (struct.pack('<4sHHQQII',b'WHK5',1,32,0x80070c81bfe375f1,10,2,0),
                        struct.pack('<4sHHQQII',b'WHV2',1,32,0x67c1043ecaa9e184,10,2,0),
                        struct.pack('<4sHHQQII',b'WHV2',1,32,0x80070c81bfe375f1,10,2,1)):
            header = copy.deepcopy(self.header)
            for a in (1,4): header['fixtures'][0]['arms'][a]['profile'] = profile.hex()
            prior = copy.deepcopy(self.old); prior['fixtures'] = copy.deepcopy(header['fixtures'])
            with self.assertRaisesRegex(ValueError,'sealed K5 descriptor'):
                C.verify_fixtures(header,prior)

    def test_old_scientific_namespace_and_work_unchanged(self):
        old=(C.ROOT/'bench/Wh2K5DeferredCostR0.cpp').read_text()
        new=(C.ROOT/'bench/Wh2K5PublicCostR0.cpp').read_text()
        normalized = new.replace('WH2_K5_PUBLIC_COST_NEUTRAL','WH2_K5_DEFERRED_COST_NEUTRAL')
        for start,end in (('int WCreate(','const Api apis'),
                          ('struct Owner {','void Initialize()'),
                          ('void Prepare(','} // namespace')):
            self.assertEqual(old[old.index(start):old.index(end)],
                             normalized[normalized.index(start):normalized.index(end)])
        # Only the candidate descriptor validation changes inside initialization.
        old_init=old[old.index('void Initialize()'):old.index('void Prepare(')]
        new_init=new[new.index('void Initialize()'):new.index('void Prepare(')]
        def strip_candidate(text):
            start=text.index('        for(unsigned a : {1u,4u})')
            end=text.index('        for(unsigned a=0;a<3;++a)',start)
            return text[:start]+text[end:]
        self.assertEqual(strip_candidate(old_init),strip_candidate(new_init))
        ...
        self.assertNotIn('wh2_small_',new)
        self.assertEqual(C.OUTPUT,Path('/var/tmp/wh2-k5-public-cost-r0'))
        self.assertEqual(C.PROTOCOL,'wirehair.wh2.k5-public-cost-r0')
        import Wh2K5DeferredCostR0 as old_reader
        for name in ('roster','statistics','clocks','multiply','selected_rows','payload',
                     'scalar_rank','verify','verify_publication','capture','run','replay'):
            self.assertEqual(inspect.getsource(getattr(C,name)),
                             inspect.getsource(getattr(old_reader,name)))
...
class ProvenanceTest(unittest.TestCase):
    def commands(self,mode):
        names = ('wirehair.cpp codec/WirehairSmall.cpp codec/WirehairSmallK5.cpp '
                 ...
                 'codec/WirehairV2Seeds.cpp codec/WirehairV2Solve.cpp').split()
        prefix = 'CMakeFiles/wirehair_objects.dir/' if mode=='native' else 'CMakeFiles/wirehair.dir/'
        options = {'native':'-O3 -DNDEBUG',
                   'scalar':'-DANDROID=1 -DWIREHAIR_SMALL_EXPECT_PORTABLE=1 -O3 -DNDEBUG',
                   'asan':'-O1 -g -fsanitize=address,undefined -fno-omit-frame-pointer -g'}[mode]
        flags = '-DWIREHAIR_BUILDING=1 '+('' if mode=='native' else '-DWIREHAIR_STATIC=1 ')
        flags += '-I'+str(C.ROOT/'include')+' '+options+' -std=gnu++11 -fPIC -Wall -Wextra -Wpedantic -Werror'
        if mode=='asan': flags += ' -march=native'
        lines=[]
        for name in names:
            obj=prefix+name+'.o'
            lines.append('/usr/bin/c++ '+flags+' -MD -MT '+obj+' -MF '+obj+'.d -o '+obj+' -c '+str(C.ROOT/name))
        lines.append(': && /usr/bin/cmake -E rm -f libwirehair.a && /usr/bin/ar qc libwirehair.a '+
                     ' '.join(prefix+name+'.o' for name in names)+' && /usr/bin/ranlib libwirehair.a && :')
        return ('\n'.join(lines)+'\n').encode()

    def test_exact_ninja_producing_recipes(self):
        for mode in ('native','scalar','asan'):
            objects,sources=C.library_commands(self.commands(mode),mode)
            self.assertEqual(len(objects),18); self.assertEqual(len(sources),18)
            self.assertEqual(objects[2].name,'WirehairSmallK5.cpp.o')
            self.assertEqual(sources[2],C.ROOT/'codec/WirehairSmallK5.cpp')

    def test_recipe_flags_order_source_archive_injection_rejected(self):
        for mode in ('native','scalar','asan'):
            raw=self.commands(mode)
            for mutate in (
                    lambda r:r.replace(b'/usr/bin/c++',b'/usr/bin/cc',1),
                    lambda r:r.replace(b' -Werror',b' -Werror -DWH_COUNT=1',1),
                    lambda r:r.replace(b' -fPIC',b' -fPIC -flto',1),
                    lambda r:r.replace(b' -fPIC',b' -fPIC -march=native',1),
                    lambda r:r.replace(b'WirehairSmallK5.cpp',b'WirehairSmall.cpp',1),
                    lambda r:r.replace(b' -c ',b' -include /tmp/overlay.h -c ',1),
                    lambda r:r.replace(b'/usr/bin/ar qc',b'/usr/bin/ar q',1),
                    lambda r:r+b'echo injected\n',
                    lambda r:r.replace(b' && /usr/bin/ranlib',b' extra.o && /usr/bin/ranlib',1),
                    lambda r:b'\n'.join(r.splitlines()[1:])+b'\n'):
                with self.subTest(mode=mode),self.assertRaises(ValueError):
                    C.library_commands(mutate(raw),mode)

    def test_exact_compiler_dep_record_and_forged_inputs(self):
        with tempfile.TemporaryDirectory(prefix='wh2-public-deps-test-') as d:
            source=Path(d)/'source.cpp'; source.write_bytes(b'source')
            header=Path(d)/'header.h'; header.write_bytes(b'header')
            target=Path('CMakeFiles/wirehair.dir/source.cpp.o')
            raw=(str(target)+': #deps 2, deps mtime 1234567890123456789 (VALID)\n'+
                 '    '+str(source)+'\n    '+str(header)+'\n\n').encode()
            self.assertEqual(C.library_dependencies(raw,target,source,1234567890123456789),{source,header})
            for mutate in (
                    lambda r:r.replace(b'(VALID)',b'(STALE)'),
                    lambda r:r.replace(b'#deps 2',b'#deps 1'),
                    lambda r:r.replace(b'1234567890123456789',b'1234567890123456788'),
                    lambda r:r.replace(b'source.cpp.o:',b'wrong.cpp.o:',1),
                    lambda r:r.replace(str(source).encode(),str(header).encode()),
                    lambda r:r.replace(str(header).encode(),b'relative.h'),
                    lambda r:r[:-1],
                    lambda r:r+b'extra\n'):
                with self.assertRaises(ValueError):
                    C.library_dependencies(mutate(raw),target,source,1234567890123456789)
            duplicate=raw.replace(b'#deps 2',b'#deps 3')[:-1]+('    '+str(header)+'\n\n').encode()
            self.assertEqual(C.library_dependencies(duplicate,target,source,1234567890123456789),{source,header})
            make=(str(target)+': '+str(source)+' \\\n '+str(header)+' '+str(header)+'\n').encode()
            self.assertEqual(C.preprocessor_dependencies(make,target),{source,header})
            for bad in (make.replace(str(target).encode(),b'wrong',1),make.replace(str(header).encode(),b'relative.h')):
                with self.assertRaises(ValueError): C.preprocessor_dependencies(bad,target)
            redirected=Path(d)/'redirect.h'; redirected.symlink_to(header)
            changed=raw.replace(str(header).encode(),str(redirected).encode())
            with patch.object(C,'ROOT',Path(d)),self.assertRaisesRegex(ValueError,'redirected'):
                C.library_dependencies(changed,target,source,1234567890123456789)

    def test_inputs_must_be_stable_across_build(self):
        with tempfile.TemporaryDirectory(prefix='wh2-public-pins-test-') as d:
            source=Path(d)/'input'; source.write_bytes(b'before')
            frozen={}; C.freeze_inputs({source},frozen)
            self.assertEqual(frozen[source],C.pin(source))
            C.freeze_inputs({source},frozen)
