...
import Wh2K5DeferredCostR0 as C
import struct
...
class CostTest(unittest.TestCase):
    ...
    def test_exact_qualified_document_not_current_substitution(self):
        old=b'qualified document\n'
        original=dict(path=str(C.ROOT/'V2_WIRE_PROFILE.md'),bytes=len(old),sha256=C.A.sha(old))
        current=dict(path=original['path'],bytes=3,sha256=C.A.sha(b'new'))
        with patch.object(C,'command',return_value=old),patch.object(C,'pin',return_value=current):
            raw,provenance=C.qualified_document(original)
            self.assertEqual(raw,old); self.assertEqual(provenance['original'],original)
            self.assertEqual(provenance['current'],current)
        for content in (b'new',old+b' '):
            with patch.object(C,'command',return_value=content),patch.object(C,'pin',return_value=current):
                with self.assertRaises(ValueError): C.qualified_document(original)
        original['path']=str(C.ROOT/'codec/WirehairSmall.cpp')
        with self.assertRaises(ValueError): C.qualified_document(original)

    def test_old_scientific_namespace_and_work_unchanged(self):
        old=(C.ROOT/'bench/Wh2K5SerializedCostR0.cpp').read_text()
        new=(C.ROOT/'bench/Wh2K5DeferredCostR0.cpp').read_text()
        for start,end in (('struct Api {','void Hex('),):
        ...
        self.assertEqual(C.OUTPUT,Path('/var/tmp/wh2-k5-deferred-cost-r0'))
