...
T = sibling('_serialization_runtime', 'Wh2ValidatedSerializationRuntimeR0.py')
P = sibling('_serialization_archive_recipes', 'Wh2K8PublicCostBuildR0.py')
A, ROOT = P.A, P.ROOT
PREPARED = Path('/tmp/wh2-validated-serialization-inputs.zloiJHko')
NEUTRAL = Path('/tmp/wh2-validated-serialization-qualified.B0LfQJ69')
...
OVERLAY = NEUTRAL/'WirehairV2ValidatedSerialization.cpp'
OBJECT = NEUTRAL/'CMakeFiles/validated_serialization_candidate.dir/WirehairV2ValidatedSerialization.cpp.o'
OBJECT_SHA = '5291f174f65d974712f92175434b90a2b0039b88ccc947fa848871c66c4284b1'
OVERLAY_SHA = 'b51ee03b2e7454d561d6e42b6abeb10b2038812b5acfeb9bb5c26ab09fe73ac6'
...
WORKER = PREPARED/'Wh2ValidatedSerializationWorker.cpp'
COMMON = PREPARED/'Wh2ValidatedSerializationCommon.h'
LINK_INPUTS = P.LINK_INPUTS+('crtbeginS.o','crtendS.o','libdl.a')
MUTABLE_HARNESS = ('bench/Wh2ValidatedSerializationCostR0.py',
                   'bench/Wh2ValidatedSerializationCostBuildR0.py',
                   'bench/test_Wh2ValidatedSerializationCostR0.py',
                   'bench/Wh2ValidatedSerializationCostR0.md',
...
def worker_sources():
    ...
    worker = exact_replace(deferred.decode(),'#include "Wh2AdmissionRegressionCostR0.cpp"',
                           '#include "Wh2ValidatedSerializationCommon.h"')
    ...
    return T.cpp_source(common).encode(),worker.encode()
...
def overlay_inputs():
    ...
    A.exact(pin(OVERLAY)['sha256'],OVERLAY_SHA,'qualified validated-local serialization overlay')
    A.exact(pin(OBJECT)['sha256'],OBJECT_SHA,'qualified native candidate')
    checks = {'Testing/Temporary/LastTest.log':'533212a82a14ac5750439b5764b62e639d9b6b933732cae031c4c873b4b6a02f',
    ...
    mechanism = {ROOT/'bench/Wh2ValidatedSerialization'/name for name in ('CMakeLists.txt','Overlay.cmake','README.md')}
    return {source,OVERLAY,OBJECT}|mechanism|{p for p in NEUTRAL.rglob('*') if p.is_file()}
...
def prepare():
    ...
    inputs.update(HERE/n for n in ('Wh2ValidatedSerializationCostR0.py','Wh2AdmissionRegressionCostR0.cpp',
    ...
    A.publish(PREPARED/'Wh2ValidatedSerializationDerivedR0.py',T.runtime_source())
...
def prepared_inputs(preparation_sha, harness_sources=()):
    ...
    derived = PREPARED/'Wh2ValidatedSerializationDerivedR0.py'
    A.exact(A.read_regular(derived,1024**2),T.runtime_source(),'exact explicit derived runtime')
    inputs.add(derived)
