#!/usr/bin/env python3
"""Run the pre-result-frozen za5v ten-K WH2 architecture screen."""
...
CANDIDATES = (
    ("dispatch10p2_frozen", 10, 2, "frozen"),
    ("pure10_frozen", 10, 0, "frozen"),
    ("pure10_tracking", 10, 0, "tracking-x"),
    ("pure9_frozen", 9, 0, "frozen"),
    ("pure9_tracking", 9, 0, "tracking-x"),
    ("pure8_frozen", 8, 0, "frozen"),
    ("pure8_tracking", 8, 0, "tracking-x"),
    ("mixed8p2_frozen", 8, 2, "frozen"),
    ("mixed8p2_tracking", 8, 2, "tracking-x"),
    ("mixed9p1_frozen", 9, 1, "frozen"),
    ("mixed9p1_tracking", 9, 1, "tracking-x"),
)
...
def descriptor_for(job):
    return band.BandDescriptor(
        staircase=band.dispatch_band_descriptor(job["K"]).staircase,
        dense_rows=4,
...
def jobs():
    result = []
    sequence = 0
    for name, gf256, gf16, x_mode in CANDIDATES:
