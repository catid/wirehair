...
import Wh2K8SerializedRecoveryR0 as C
...
def arm_result(e,arm):
    ...
    if arm in (0,3):
        ...
        profile=struct.pack('<4sHHQQII',b'WHK8',1,32,0x5748324b38544d31,size,width,0)
...
class RecoveryTest(unittest.TestCase):
    ...
    def test_all_arm_descriptors_packets_and_ledgers(self):
        ...
        self.reject(lambda r:r[4]['arms'][1].__setitem__('profile',r[0]['arms'][1]['profile']),'WHK8 identity')
    ...
    def test_spent_namespace_does_not_launch(self):
        with tempfile.TemporaryDirectory(prefix='wh2-k8-recovery-spent-') as d:
    ...
    def test_backend_mismatch_seals_invalid_and_preserves_prefix(self):
        with tempfile.TemporaryDirectory(prefix='wh2-k8-backend-parity-') as d:
    ...
    def test_partial_spool_preserves_existing_file(self):
        with tempfile.TemporaryDirectory(prefix='wh2-k8-recovery-spool-') as d:
    ...
    def test_output_cap_timeout_and_cleanup(self):
        ...
        for code,cap,deadline in [('print("x"*100,flush=True)',32,2),
            ...
            with tempfile.TemporaryDirectory(prefix='wh2-k8-recovery-cap-') as d:
