...
constexpr char protocol[]="wirehair.wh2.k8-public-recovery-r0";
constexpr char claim_path[]="/var/tmp/wh2-k8-public-recovery-r0/CLAIM.json";
}
...
#if !defined(WH2_K8_PUBLIC_RECOVERY_BACKEND) || defined(WH_COUNT) || defined(WIREHAIR_TESTING) || defined(WIREHAIR_V2_ENABLE_TEST_HOOKS)
#error "Explicit unmodified recovery backend required"
#endif
static_assert(small_recovery_config::K==8, "sealed installed K8");
static_assert(WH2_K8_PUBLIC_RECOVERY_BACKEND>=0 && WH2_K8_PUBLIC_RECOVERY_BACKEND<=2, "backend");
namespace {
using Byte=uint8_t;
...
// Actual WHV2 candidate boundary. No benchmark facade or descriptor translation.
...
int SIndependent(const void* s,unsigned message,unsigned b,Profile& p,void*& h) {
    return SCreate(s,message,b,p,h,WirehairV2EncoderSource_Independent);
}
int SBorrowed(const void* s,unsigned message,unsigned b,Profile& p,void*& h) {
    return SCreate(s,message,b,p,h,WirehairV2EncoderSource_BorrowedImmutable);
}
int SEncode(void* h,uint32_t id,void* out,unsigned b,uint32_t* n) { return PEncode(h,id,out,b,n); }
int SDecoder(const Profile& p,unsigned message,unsigned b,void*& h) { return PDecoder(p,message,b,h); }
int SFeed(void* h,uint32_t id,const void* in,unsigned n) { return PFeed(h,id,in,n); }
// Exercise calls Recover only after Success; no error-field equivalence claimed.
int SRecover(void* h,void* out,unsigned n,uint64_t* written) { return PRecover(h,out,n,written); }
void SFree(void* h) { PFree(h); }
    ...
    for(unsigned a=0;a<arms;++a) {
        ...
        if(a==1 || a==4) {
            ...
            Check(wirehair_v2_profile_deserialize(r.profile.data(),32,&p)==WirehairV2_Success &&
                  p.profile_id==WIREHAIR_V2_PROFILE_SMALL_K8_2026_09 && p.seed_attempt==0 &&
                  p.message_bytes==source.size() && p.block_bytes==B,"installed WHV2 candidate descriptor");
        }
        if(a==0 || a==3) {
            WirehairV2Profile p={};
            Check(wirehair_v2_profile_deserialize(r.profile.data(),32,&p)==WirehairV2_Success &&
...
gf256_x86_cpu_features Initialize(const Oracle& oracle) {
    ...
    if(WH2_K8_PUBLIC_RECOVERY_BACKEND==1) Check(!f.SSSE3 && !f.AVX2 && !f.GFNI && !f.AVX512,"portable backend");
...
void Header(const std::string& claim,const char* scope,const gf256_x86_cpu_features& f) {
    printf("{\"type\":\"header\",\"protocol\":\"%s\",\"claim\":\"%s\",\"backend\":\"%s\",\"scope\":\"%s\","
           ...
           protocol,claim.c_str(),backends[WH2_K8_PUBLIC_RECOVERY_BACKEND],scope,small_recovery_data::kRawSha,
...
int Worker(const std::string& claim) {
    ...
    if(WH2_K8_PUBLIC_RECOVERY_BACKEND!=2) Check(!setrlimit(RLIMIT_AS,&memory),"worker memory cap");
...
void DescriptorNegatives() {
    const auto source=Message(2,1);
    Profile original={}; Ledger ledger={};
    Owner encoder(apis[1],ledger);
    Check(SIndependent(source.data(),unsigned(source.size()),2,original,encoder.handle)==0 &&
          encoder.handle,"neutral descriptor encoder");
    for(unsigned kind=0;kind<4;++kind) {
        Profile altered=original;
        if(kind==0) {
            memcpy(altered.data(),"WHK8",4);
            const uint64_t id=UINT64_C(0x5748324b38544d31);
            for(unsigned i=0;i<8;++i) altered[8+i]=Byte(id>>(8*i));
        } else if(kind==1) altered[8]^=1;
        else if(kind==2) altered[28]=1;
        else altered[29]=1;
        WirehairV2Profile parsed={};
        Check(wirehair_v2_profile_deserialize(altered.data(),32,&parsed)!=WirehairV2_Success,
              "invalid installed profile rejected");
        Owner decoder(apis[1],ledger);
        Check(SDecoder(altered,unsigned(source.size()),2,decoder.handle)!=0 && !decoder.handle,
              "invalid installed decoder rejected without handle");
    }
}
    ...
    try {
        if(argc==2 && !strcmp(argv[1],"--contract")) {
            printf("{\"K\":%u,\"arms\":%u,\"records\":%u,\"cpu_seconds\":%u,\"wall_seconds\":%u,\"address_space_mib\":%u,\"asan_shadow_exempt\":%s,\"backend\":\"%s\",\"claim_path\":\"%s\"}\n",
                   K,arms,small_recovery_config::records,cpu_seconds,wall_seconds,address_space_mib,WH2_K8_PUBLIC_RECOVERY_BACKEND==2?"true":"false",
                   backends[WH2_K8_PUBLIC_RECOVERY_BACKEND],claim_path); return 0;
        }
