...
static const char Name[] = "wh2-public-stack-crossover-r0";
static const char Prefix[] = "wirehair.wh2.public-stack-crossover-r0";
static const char Directory[] = "/var/tmp/wh2-public-stack-crossover-r0";
...
void Run(Api& baseline, const Clock::time_point& deadline)
{
    ...
    baseline.Stable(); Require(old::FileHash(Libc)==LibcHash,"libc changed");
    ...
    const std::string& workerHash, const std::string& descriptionHash)
{
    ...
    Require(old::Field(claim,"baseline_path")==Baseline && old::Field(claim,"baseline_sha256")==BaselineHash &&
        old::FileHash(Libc)==LibcHash,"baseline/libc authorization differs");
...
{
    using namespace stack_crossover_r0;
    try {
#if defined(__linux__) && defined(__x86_64__) && !defined(__ILP32__) && \
    defined(__GNUC__) && __GNUC__ == 13 && !defined(__clang__)
        if (argc==2 && std::string(argv[1])=="--describe") std::cout<<Describe();
        else if (argc==2 && std::string(argv[1])=="--selftest") {
            Selftest(); std::cout<<"stack-crossover-r0 pure selftest passed (no codec or clocks)\n";
        } else {
...
#endif
    } catch (const std::exception& error) { std::cerr<<"stack-crossover-r0: "<<error.what()<<'\n'; return 1; }
}
