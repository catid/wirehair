...
import Wh2K3OrdinaryRecoveryR0 as C
...
def synthetic():
    ...
    for e in C.roster():
        ...
        arms = []
        for arm in range(4):
            first = C.candidate_first(tuple(ids)) if arm in (1,3) else (
                0 if e['index'] % (17+arm) == 0 else min(len(ids), 4+arm))
            profile = (struct.pack('<4sHHQQII', b'WHV2', 1, 32, 0x4b295bbb47f4f9c9, 3*width, width, 0) if arm == 0 else
                       struct.pack('<4sHHQQII', b'WHV2', 1, 32, 0x67c1043ecaa9e184, 3*width, width, 0) if arm in (1,3) else
                       b'\0'*32)
            packets = [C.candidate_packet(width, i) for i in ids] if arm in (1,3) else ['00'*32]*len(ids)
...
class RecoveryTest(unittest.TestCase):
    ...
    def test_complete_synthetic_three_backends(self):
        ...
        self.assertEqual(expected['groups'][0]['failure_counts_oh0_to_4'][3], [0]*5)
        self.assertEqual(expected['policy_oh0_wins'],
                         dict(ordinary_independent=True, ordinary_borrowed=True))
        self.assertFalse(expected['production_promotion_claimed'])
    ...
    def test_paired_fixes_and_introductions(self):
        ...
        for arm, paired in zip((0, 2, 0, 2), fresh['paired']):
    ...
    def test_both_policies_introductions_are_counted(self):
        records = copy.deepcopy(self.records)
        for arm in (0, 2):
            records[0]['arms'][arm].update(first=3, feed=[1,1,0], recoveries=2)
        for arm in (1, 3):
            records[0]['arms'][arm].update(first=0, feed=[1]*7, recoveries=0)
        result = C.summarize(records, C.roster())
        paired = result['groups'][0]['paired']
        self.assertEqual(len(paired), 4)
        self.assertEqual([(p['candidate'], p['control']) for p in paired],
                         [(C.ARMS[c], C.ARMS[a]) for c, a in ((1,0), (1,2), (3,0), (3,2))])
        key = [records[0]['index'], records[0]['B']]
        for p in paired:
            self.assertEqual(p['introduced'], [[key]]*5)
            for fixed in p['fixed']:
                self.assertNotIn(key, fixed)

    def test_all_four_arms_are_required(self):
        self.reject(lambda rows: rows[0]['arms'].pop(), 'four actual API routes')

    def test_control_cannot_select_the_new_default(self):
        def change(rows):
            rows[0]['arms'][0]['profile'] = rows[0]['arms'][1]['profile']
        self.reject(change, 'explicit certified WH2 descriptor')

    def test_borrowed_policy_cannot_escape_the_oracles(self):
        self.reject(lambda rows: rows[0]['arms'][3]['packets'].__setitem__(0, '00'*32),
                    'independent K3 payload hashes')
        def change(rows):
            rows[0]['arms'][3].update(first=0, feed=[1]*7, recoveries=0)
        self.reject(change, 'independent first-success rank')

    def test_historical_prefixes_are_never_extended(self):
        rows = copy.deepcopy(self.records)
        historical = next(r for r in rows if r['group'] == 2 and len(r['ids']) < 7)
        historical['ids'].append(0)
        with self.assertRaisesRegex(ValueError, 'retained exact chronology'):
            C.verify(raw(rows), '0'*64, 'native')
