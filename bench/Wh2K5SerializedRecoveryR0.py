#!/usr/bin/env python3
"""Frozen K5 serialized recovery, six ownership-matched arms; retained cohort only."""
import argparse
...
U = sibling('_k5_recovery_build_io', 'Wh2K5SerializedCostR0.py')
D = sibling('_k5_recovery_retained_data', 'Wh2K5NativeDataR0.py')
A = U.A
ROOT = U.ROOT
PROTOCOL = 'wirehair.wh2.k5-serialized-recovery-r0'
OUTPUT = Path('/var/tmp/wh2-k5-serialized-recovery-r0')
...
SOURCES = ('bench/Wh2K5SerializedRecoveryR0.cpp', 'bench/Wh2K5SerializedRecoveryR0.py',
           'bench/test_Wh2K5SerializedRecoveryR0.py', 'bench/Wh2K5SerializedCostR0.py',
    ...
    for row,e in zip(records,expected):
        ...
        for arm,r in enumerate(row['arms']):
            ...
            if arm in (0,3):
                ...
                A.exact(profile,struct.pack('<4sHHQQII',b'WHK5',1,32,0x5748324b35544d31,K*width,width,0),'sealed WHK5 identity')
...
def qualified_document(original):
    ...
    old = U.command(['git','cat-file','blob','d59758990d05cea9c76924da6974ca53843c80f6:V2_WIRE_PROFILE.md'])
    A.exact(dict(path=str(path),bytes=len(old),sha256=A.sha(old)),original,'exact qualified Git document')
    return old,dict(original=original,current=U.pin(path),
...
def qualified_inputs(mode):
    path = U.SMALL/'artifact-inventory.json'
    raw = A.read_regular(path,1024*1024)
    A.exact(A.sha(raw),U.INVENTORY_SHA,'qualified K5 engineering inventory')
    ...
    for original in inventory['files']:
        ...
        if name==str(ROOT/'V2_WIRE_PROFILE.md'):
            ...
            A.exact(U.pin(Path(name)),original,'unchanged qualified producing artifact')
    ...
    archives = [U.SMALL/mode/'libwh2_small_serialized.a',U.QUALIFIED/mode/'libwirehair.a']
    for archive in archives: A.require(str(archive) in declared,'bound qualified archive')
    A.exact(U.pin(archives[1])['sha256'],U.ARCHIVES[mode],'actual WH1/certified-WH2 library')
    for name in ('WirehairSmall.cpp.o','WirehairV2Profile.cpp.o'):
        old = U.QUALIFIED/'native/CMakeFiles/wirehair_objects.dir/codec'/name
        A.exact(U.pin(U.SMALL/name)['sha256'],U.pin(old)['sha256'],'unchanged production object proof')
    log = A.read_regular(U.SMALL/mode/'Testing/Temporary/LastTest.log',1024*1024).decode()
...
def build(mode, output):
    ...
    flags = ['-std=c++11', '-Wall', '-Wextra', '-Wpedantic', '-Werror', '-fno-lto', '-fPIC',
             '-DWIREHAIR_STATIC=1', '-DWH2_SMALL_CODEC_K=5', '-DWH2_K5_SERIALIZED_RECOVERY_BACKEND='+str(MODES.index(mode)),
    ...
    for prefix in ('wirehair_', 'wirehair_v2_', 'wh2_small_'):
