...
import Wh2K3OrdinaryCostR0 as C
import struct
...
def synthetic():
    ...
    for b in C.WIDTHS:
        ...
        generic = dict(profile='00'*32, packets='00'*(18*b), steps=[3, 3])
        candidate = dict(profile=candidate_profile, packets=candidate_packets, steps=[3, 3])
        fixtures.append(dict(width=b, source=bytes((37*i+i//11) % 256 for i in range(3*b)).hex(),
                             arms=[copy.deepcopy(generic), candidate, copy.deepcopy(generic), copy.deepcopy(candidate)]))
        fixtures[-1]['arms'][0]['profile'] = struct.pack('<4sHHQQII', b'WHV2', 1, 32, 0x4b295bbb47f4f9c9, 3*b, b, 0).hex()
    ...
    for coordinate in C.roster():
        ...
        target = ready+q; duration = (100000, 99000, 120000, 98000)[arm]
        ...
        counts = [0 if metric else 128, 0 if metric else 2304, 128 if metric else 0,
                  384 if metric else 0, 128 if metric else 0, 128]
...
class CostTest(unittest.TestCase):
    ...
    def test_whole_synthetic_stream(self):
        ...
        self.assertEqual(len(result['statistics']), 144)
    ...
    def test_roster_independent_coverage(self):
        ...
        self.assertEqual(len(counts), 1728)
        self.assertEqual(set(counts.values()), {1})
        self.assertEqual(len(phases), 6912)
    ...
    def test_candidate_oracle_and_descriptor_are_required(self):
        ...
        header['fixtures'][0]['arms'][3]['packets'] = '00'*36
        old = copy.deepcopy(self.old)
        old['fixtures'][0]['arms'][1]['packets'] = '00'*36
        old['fixtures'][0]['arms'][3]['packets'] = '00'*36
        with self.assertRaisesRegex(ValueError, 'independent selected-pair payload oracle'):
    ...
    def test_target_fixture_and_schema_must_match(self):
        ...
        for change in (lambda h: h.update(protocol='wrong'), lambda h: h.update(batch=True),
                       lambda h: h['fixtures'][0]['arms'][0]['steps'].__setitem__(0,4),
    ...
    def test_both_candidate_policies_use_identical_equations(self):
        for field in ('profile', 'packets', 'steps'):
            header = copy.deepcopy(self.header)
            header['fixtures'][0]['arms'][3][field] = header['fixtures'][0]['arms'][0][field]
            if field == 'steps':
                header['fixtures'][0]['arms'][3][field] = [4, 3]
            old = copy.deepcopy(self.old)
            old['fixtures'] = copy.deepcopy(header['fixtures'])
            with self.assertRaisesRegex(ValueError, 'policy-independent complete fixture'):
                C.verify_fixtures(header, old)
