...
import Wh2K6SerializedCostR0 as C
...
def synthetic():
    ...
    for b in C.WIDTHS:
        ...
        generic = dict(profile='00'*32, packets='00'*(18*b), steps=[6, 6])
        candidate = dict(profile=candidate_profile, packets=candidate_packets, steps=[6, 6])
        ...
        handles = [dict(arm=a, profile_hex='00'*32, decode=[dict(feeds=[0]*6)]*2) for a in (1,2,3)]
        old.append(dict(handles=handles, packets_hex=[candidate_packets, candidate_packets, generic['packets'], generic['packets']]))
...
class CostTest(unittest.TestCase):
    ...
    def test_candidate_oracle_and_descriptor_are_required(self):
        ...
        old['fixtures'][0]['packets_hex'][1] = '00'*36
