"""Neutral tests only: unrelated matrices and synthetic K4 outcomes."""
import hashlib
...
M=sibling('k4_tm_tested','Wh2K4ThueMorseR0.py');R=M.R
OLD=sibling('k4_controller_tests','test_Wh2NoncommutingRadixRunR0.py');OLD.M=R.C
NEUTRAL=(5,7,11,13)
...
class Tests(unittest.TestCase):
    def test_field_and_rank(self):
        ...
        for i in range(256):
            raw=hashlib.sha512(('k4-neutral/'+str(i)).encode()).digest()
            rows=[raw[j:j+4] for j in range(0,16,4)]
            self.assertEqual(M.checked_rank(rows),M.G.reference_rank(rows))
        for rows in ([],[(0,)*4],M.K.identity(4),[(1,)+(0,)*3]*8):
            self.assertEqual(M.checked_rank(rows),M.G.reference_rank(rows))
        with mock.patch.object(M.F,'matrix_rank',return_value=0):
            with self.assertRaisesRegex(ValueError,'rank disagreement'):
                M.checked_rank(M.K.identity(4))

    def test_unrelated_dimension4_mapper(self):
        ...
        self.assertEqual(evidence['lookup_bytes'],20480)
        ...
        with mock.patch.object(mapper,'payload',bytes(20479)):
    ...
    def test_local_selection_first_success_and_exhaustion(self):
        records=[]
        with mock.patch.object(M.F,'matrix_rank',side_effect=[0,4]), \
             mock.patch.object(M,'trace') as trace,mock.patch.object(R,'inventory_inputs') as history:
            pair=M.G.choose_pair(NEUTRAL,('0',),(tuple(range(4)),),mock.Mock(),records)
        ...
        with mock.patch.object(M.F,'matrix_rank',return_value=0):
            records=[]
            self.assertIsNone(M.G.choose_pair(NEUTRAL,('0',),(tuple(range(4)),),mock.Mock(),records))
    ...
    def test_binary_factors_and_minor_roster(self):
        ...
        self.assertEqual(len(M.MINORS),70)
        self.assertEqual(len(set(M.MINORS)),70)
        self.assertEqual(M.MINORS,tuple(itertools.combinations(range(8),4)))

    def test_integer_trace_reference(self):
        def reference(width,root,kind):
            state=(int(root,16)^4*0x9e3779b97f4a7c15^width*0xbf58476d1ce4e5b9)%(2**64)
            if kind!='iid':state^=0x10fade
            threshold=(1/9 if kind=='burst' else .1 if kind=='iid' else .5)*2**53
            ids=[];skip=0
            for n in range(67584):
                ...
                if ((v^(v>>31))>>11)<threshold:
                    ...
                    ids.append(2**32-1-2*n if kind=='adversarial' else 4+n if kind=='repair-only' else n)
                    if len(ids)==8:return ids
    ...
    def test_collisions_and_exclusion_roster_without_scoring(self):
        ...
        self.assertEqual(len(hashes),5*512)
    ...
    def pipeline(self,failure=None):
        ...
        class FakeMapper:
            ...
            def row(self,i):
                value=M.K.identity(4)[i%4];self.cache[i]=value;return value
        trace_calls=0
        def trace(b,root,schedule,mapper):
            ...
            return dict(B=b,root=root,schedule=schedule,ids=list(range(8)),ranks=[3,4,4,4,4] if fail else [4]*5)
        rank_calls=0
        def rank(rows):
            nonlocal rank_calls
            rank_calls+=1
            # Mapper verification is independently exercised with unrelated real matrices.
            # 700 local minors, 2170 seam minors, then 37 historical prefixes.
            return 3 if (failure,rank_calls) in (('local',1),('seam',701),('history',2871)) else 4
        with mock.patch.object(M,'fixed_feedback',return_value=NEUTRAL), \
             ...
             mock.patch.object(R,'inventory_inputs',return_value=dict(prefixes=[dict(ids=list(range(4)),original_widths=[64])]*37,roots=[])), \
        ...
        self.assertEqual(result['counts']['local_minors'],700)
        self.assertEqual(result['counts']['seam_minors'],2170)
        self.assertEqual(result['counts']['history_prefixes'],37)
        self.assertEqual([r['ids'][0] for r in result['seams']],[(1<<e)-4 for e in range(2,32)]+[2**32-8])
        self.assertTrue(all(len(r['ids'])==8 for r in result['seams']))
    ...
    def test_actual_retained_projection_and_corruption(self):
        ...
        self.assertEqual((len(actual['origins']),len(actual['prefixes']),len(actual['roots'])),(38,37,64))
        self.assertTrue(all(4<=len(p['ids'])<=8 and p['original_widths'] for p in actual['prefixes']))
        ...
        with mock.patch.dict(R.PROJECTIONS,origins=(38,'0'*64)):
