#!/usr/bin/env python3
"""Frozen dimension-four GF256 feasibility screen, never native timing."""
...
G=sibling('k4_generic_selection','Wh2K5ThueMorseR0.py')
R=sibling('k4_frozen_inputs','Wh2K4ThueMorseRunR0.py')
K,F,require=G.K,G.F,G.require
PROTOCOL=R.C.PROTOCOL
N,LENGTH,CANDIDATE_LIMIT=4,8,67584
...
def verify_mapper(pair,mapper,budget):
    ...
    require(len(mapper.payload)==20480,'packed K4 geometry')
    require(tuple(mapper.row(i) for i in range(N))==K.identity(N),'systematic K4 rows')
    ...
    for b,schedule in itertools.product(WIDTHS,SCHEDULES):
        ...
        for r in cell:
            require(len(r['ranks'])==5 and all(type(v) is int and 0<=v<=N for v in r['ranks']) and
                    all(0<=y-x<=1 for x,y in zip(r['ranks'],r['ranks'][1:])),'nested K4 ranks')
...
def run_screen(claim):
    ...
    try:
        ...
        for word in WORDS:
            ...
            result['local'].append(dict(word=word,columns=columns,checked=70))
        mapper=K.Mapper(pair,budget)
        result['evidence'].update(verify_mapper(pair,mapper,budget))
        for start in [(1<<e)-4 for e in range(2,32)]+[K.MAX_ID-LENGTH+1]:
        ...
        result['counts']=dict(local_minors=700,seam_minors=2170,hard_traces=len(result['hard']),
            history_prefixes=len(result['history']),fresh_traces=len(result['fresh']),unique_rows=len(mapper.cache))
        require(len(result['local'])==10 and len(result['seams'])==31 and len(result['hard'])==72 and
                len(result['history'])==37 and len(result['fresh'])==(6144 if structural else 0),'complete screen accounting')
        budget.check()
...
def main(argv):
    require(argv==['--worker'],'usage: Wh2K4ThueMorseR0.py --worker')
