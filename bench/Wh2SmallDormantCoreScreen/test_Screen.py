...
def groups():
    return {(c,p,o):[math.log(0.95) if p >= 3 else 0.0]*12
            for c in range(180) for p in range(5) for o in range(2)}
...
class Test(unittest.TestCase):
    def test_complete(self):
        ...
        self.assertEqual((r['rows'],len(r['statistics']),len(r['primary'])),(388800,1800,6))
        self.assertEqual(len(r['fixed_addresses']),2160)
        self.assertEqual(len(r['first_success']),360)
    ...
    def test_roster(self):
        self.assertEqual(len(list(S.roster())),388800)
        self.assertEqual([S.shape(c)['route'] for c in (0,30,60,90,120,150)],list(range(6)))
        self.assertEqual([S.shape(c)['k'] for c in (0,30,60,90,120,150)],[3,3,5,8,3,16])
        self.assertEqual(sum(S.shape(c)['small'] for c in range(180)),120)
        for c in (-1,180,1.0):
