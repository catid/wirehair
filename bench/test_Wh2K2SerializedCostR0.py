...
import Wh2K2SerializedCostR0 as C
import struct
...
def synthetic():
    ...
    for b,tail in C.SHAPES:
        ...
        generic = dict(profile='00'*32, packets=candidate_packets, steps=[2,2], rows=rows, fallback=[False,False])
        candidate = dict(profile=candidate_profile, packets=candidate_packets, steps=[2,2], rows=rows, fallback=[False,False])
        certified = copy.deepcopy(generic)
        certified['profile'] = struct.pack('<4sHHQQII',b'WHV2',1,32,0x4b295bbb47f4f9c9,b+tail,b,0).hex()
        arms = [certified,candidate,generic]
        fixtures.append(dict(width=b,message=b+tail,packet_bytes=C.packet_lengths(b,tail),source=bytes((37*i+i//11) % 256 for i in range(b+tail)).hex(),
    ...
    for coordinate in C.roster():
        ...
        counts = [0 if metric else 128, 0 if metric else 1792, 128 if metric else 0,
                  256 if metric else 0, 128 if metric else 0, 128]
...
class CostTest(unittest.TestCase):
    ...
    def test_candidate_oracle_and_descriptor_are_required(self):
        ...
        header = copy.deepcopy(self.header)
        header['fixtures'][0]['arms'][1]['packets'] = '00'*28
        header['fixtures'][0]['arms'][4]['packets'] = '00'*28
        old = copy.deepcopy(self.old)
        old['fixtures'][0]['arms'][1]['packets'] = '00'*28
        old['fixtures'][0]['arms'][4]['packets'] = '00'*28
    ...
    def test_required_frozen_sizes_and_ownership_pairs(self):
        ...
        self.assertEqual(C.ARM_NAMES,('ordinary_independent','k2_independent','wh1_owned',
                                     'ordinary_borrowed','k2_borrowed','wh1_borrowed'))
        self.assertEqual(len(C.selected_rows()),14)
    ...
    def test_old_scientific_namespace_and_work_unchanged(self):
        ...
        new=(C.ROOT/'bench/Wh2K2SerializedCostR0.cpp').read_text()
        ...
        self.assertEqual(C.OUTPUT,Path('/var/tmp/wh2-k2-serialized-cost-r0'))

    def test_partial_tail_bytes_and_guards(self):
        for shape,(b,tail) in enumerate(C.SHAPES):
            f=self.header['fixtures'][shape]
            self.assertEqual(f['message'],b+tail)
            self.assertEqual(f['packet_bytes'],[b,tail]+[b]*12)
            packets=bytes.fromhex(f['arms'][1]['packets'])
            self.assertEqual(packets[b+tail:2*b],bytes([0xa5])*(b-tail))
            if tail<b:
                changed=copy.deepcopy(self.header)
                changed['fixtures'][shape]['arms'][1]['packets']=(
                    packets[:b+tail]+b'\x00'+packets[b+tail+1:]).hex()
    ...
    def test_actual_feed_endpoint_and_fallback_are_charged(self):
        ...
        # Controls may need all six repairs followed by two systematic packets.
        # Never equalize their work to the candidate's two-packet endpoint.
        rows=[(1,0),(0,1)]+[(1,0)]*12
        for arm in (0,3):
            f['arms'][arm].update(rows=bytes(v for row in rows for v in row).hex(),
                packets=C.payload(rows,b,tail),steps=[8,8],fallback=[True,True])
        prior=copy.deepcopy(self.old);prior['fixtures']=copy.deepcopy(header['fixtures'])
        C.verify_fixtures(header,prior)
        records=copy.deepcopy(self.records)
        for record in records:
            c=record['coordinate']
            if c[3]==1 and c[4]>0 and c[7] in (0,3):record['counts'][3]=128*8
