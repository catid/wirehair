...
import Wh2K3ProductionCostR0 as C
import struct
...
def synthetic():
    ...
    for b in C.WIDTHS:
        ...
        fixtures.append(dict(width=b, source=bytes((37*i+i//11) % 256 for i in range(3*b)).hex(),
                             arms=[copy.deepcopy(generic), candidate, copy.deepcopy(generic)]))
        fixtures[-1]['arms'][0]['profile'] = struct.pack('<4sHHQQII', b'WHV2', 1, 32, 1, 3*b, b, 0).hex()
