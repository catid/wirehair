#!/usr/bin/env python3
"""Sealed same-work MIX2 iterator-pair Stage 0 screen.

This controller runs only pairs 02 and 12 on the three preregistered weak
coordinates already observed by the v9 attempt map.  It consumes no new root,
collects no timing evidence, and cannot promote a profile.  A surviving pair
only earns the separately frozen consumed-root Stage 1 replay.
"""
...
CONTRACT_SCHEMA = "wirehair.wh2.mix2-pair-stage0-contract.v1"
RECORD_SCHEMA = "wirehair.wh2.mix2-pair-stage0-record.v1"
SUMMARY_SCHEMA = "wirehair.wh2.mix2-pair-stage0-summary.v1"
BENCH_DESCRIPTION_SCHEMA = "wirehair.wh2.v2-bench-description.v1"
RECORD_NAME = "mix2-pair-stage0-records.jsonl"
SUMMARY_NAME = "mix2-pair-stage0-summary.json"
...
EXPECTED_INVOCATION_COUNT = len(PAIRS) * len(COORDINATES) * len(REPETITIONS)
...
SOURCE_STATUS_PATHS = (
    ...
    "WirehairPeelFixups.inc", "bench/wh2_mix2_pair_stage0.py",
    "bench/wh2_mix2_promotion_short_screen.py",
)
...
def _contract_body() -> Mapping[str, Any]:
    return {
        ...
        "bead": "wirehair-sxvz.16.1.20.38.1",
        "scope": "same-work fixed MIX2 iterator-pair Stage 0 only",
        ...
        "candidate": {
            ...
            "attempt_rule": "exact precode attempt equals packet attempt",
        },
        "domain": {
            "coordinates": [
        ...
        "gate": {
            ...
            "priority_order": list(PAIRS),
            "survivors": "ordered subset of [02,12] passing every per-pair gate",
            "stage1_candidate": (
                "first survivor in frozen priority order [02,12], or null; "
                "only this pair is licensed for the 1080-cell consumed-root "
                "Stage 1 replay"),
...
def contract_description() -> Mapping[str, Any]:
    ...
    if digest != EXPECTED_CONTRACT_SHA256:
        fail("MIX2 pair Stage 0 contract differs from its frozen digest")
    body["contract_sha256"] = digest
    return body


def _validate_constants() -> None:
    if PAIRS != ("02", "12") or REPETITIONS != (0, 1):
        fail("pair or repetition roster changed")
    ...
    if EXPECTED_INVOCATION_COUNT != 12:
...
class Invocation:
    ...
    def __post_init__(self) -> None:
        if type(self.ordinal) is not int or not 0 <= self.ordinal < 12:
            fail("invocation ordinal is outside Stage 0")
        if type(self.pair) is not str or self.pair not in PAIRS:
            fail("invocation MIX2 pair is outside Stage 0")
        ...
        expected_ordinal = (
            (PAIRS.index(self.pair) * len(COORDINATES) +
    ...
    def argv(self, bench: Path) -> List[str]:
        ...
        return [
            ...
            "--mix-pair", self.pair, "--binary-dense-rows", "12",
        ...
        for coordinate_ordinal in range(len(COORDINATES)):
            for repetition in REPETITIONS:
                invocations.append(Invocation(
                    len(invocations), pair, coordinate_ordinal, repetition))
...
DETERMINISTIC_FIELDS = (
    "pair", "coordinate_ordinal", "K", "attempt", "root_index",
...
def adjudicate(records: Sequence[Mapping[str, Any]]) -> Mapping[str, Any]:
    if len(records) != EXPECTED_INVOCATION_COUNT:
    ...
    for expected_ordinal, record in enumerate(records):
        ...
        try:
            key = (record["pair"], record["coordinate_ordinal"],
                   record["repetition"])
    ...
    if set(by_key) != expected or ordinals != set(range(12)):
    ...
    for pair in PAIRS:
        pair_records = [
            by_key[(pair, coordinate, repetition)]
            for coordinate in range(len(COORDINATES))
            for repetition in REPETITIONS
        ]
        exact_coordinates = 0
        ...
        coordinate_results = []
        for coordinate in range(len(COORDINATES)):
            first = by_key[(pair, coordinate, 0)]
            second = by_key[(pair, coordinate, 1)]
        ...
        successes = sum(int(record["success"]) for record in pair_records)
        rank_failures = sum(int(record["rank_fail"]) for record in pair_records)
        errors = sum(int(record["error"]) for record in pair_records)
        configuration_failures = sum(
            int(record["configuration_failure"]) for record in pair_records)
        passed = (
    ...
    return {
        ...
        "survivors": survivors,
        "stage1_candidate": survivors[0] if survivors else None,
...
def run_stage0(bench: Path, output_dir: Path) -> Mapping[str, Any]:
    _validate_constants()
    ...
    with screen._open_binary(bench, "wirehair_v2_bench") as pinned:
        return _run_stage0_pinned(
            pinned, output_dir, source, contract)
...
def _run_stage0_pinned(
    ...
    verdict = adjudicate(records)

    temporary = Path(tempfile.mkdtemp(
        prefix=".wh2-mix2-pair-stage0-", dir=str(output_dir.parent)))
    published = False
    try:
        ...
        if pinned.receipt() != bench_receipt or _source_receipt() != source:
...
def _parser() -> argparse.ArgumentParser:
    ...
    run_parser = subparsers.add_parser("run", help="run the 12-cell Stage 0")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    try:
        ...
        if arguments.command == "describe":
            ...
            return 0
        summary = run_stage0(arguments.bench, arguments.output_dir)
        print(canonical_json(summary))
        return 0 if summary["disposition"] == "PASS" else 2
    except (Stage0Error, screen.ShortScreenError) as exc:
        print("wh2 MIX2 pair Stage 0: {}".format(exc), file=sys.stderr)
        return 1
