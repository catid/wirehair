// Retained ordinary WH2 K3 recovery under both source policies versus WH1/certified WH2.
...
#if !defined(WH2_K3_ORDINARY_RECOVERY_BACKEND) || defined(WH_COUNT) || defined(WIREHAIR_TESTING) || defined(WIREHAIR_V2_ENABLE_TEST_HOOKS)
#error "Explicit unmodified recovery build required"
#endif
static_assert(WH2_K3_ORDINARY_RECOVERY_BACKEND >= 0 && WH2_K3_ORDINARY_RECOVERY_BACKEND <= 2, "backend");
namespace {
using Byte = uint8_t;
using Profile = std::array<Byte,32>;
const char protocol[] = "wirehair.wh2.k3-ordinary-recovery-r0";
...
using Result = std::array<ArmResult,4>;
Result Exercise(const Oracle& oracle,unsigned B,const uint32_t* ids,unsigned count) {
    ...
    for(unsigned a=0;a<4;++a) {
        ...
        std::vector<std::vector<Byte>> packets(count,std::vector<Byte>(B+2,0xa5));
        {
            ...
            for(unsigned i=0;i<count;++i) {
                ...
                if(a==1 || a==3) Check(!memcmp(packets[i].data()+1,independent[i].data(),B),"independent polynomial payload");
        ...
        if(a!=2) {
            ...
            Check(wirehair_v2_profile_deserialize(r.profile.data(),32,&profile)==WirehairV2_Success &&
                  profile.profile_id==(a==0?WIREHAIR_V2_PROFILE_CERTIFIED_2026_07:
                                             WIREHAIR_V2_PROFILE_SMALL_K3_2026_09),"explicit equation profile");
        }
        Owner decoder(api);
        ...
        if(a==1 || a==3) Check(r.first==candidate_first,"independent first-success rank");
    ...
    Check(result[1].profile==result[3].profile && result[1].packets==result[3].packets &&
          result[1].feed==result[3].feed && result[1].first==result[3].first &&
          result[1].recoveries==result[3].recoveries,"policy-independent complete result");
    return result;
}
...
gf256_x86_cpu_features Initialize(const Oracle& oracle) {
    ...
    if(WH2_K3_ORDINARY_RECOVERY_BACKEND==1) Check(!f.SSSE3 && !f.AVX2 && !f.GFNI && !f.AVX512,"portable backend");
    return f;
}
uint64_t Now() {
    timespec t={};
...
int Worker(const std::string& claim) {
    ...
    std::ifstream in("/var/tmp/wh2-k3-ordinary-recovery-r0/CLAIM.json",std::ios::binary);
    ...
    if(WH2_K3_ORDINARY_RECOVERY_BACKEND!=2) Check(!setrlimit(RLIMIT_AS,&memory),"worker memory cap");
    ...
    printf("{\"type\":\"header\",\"protocol\":\"%s\",\"claim\":\"%s\",\"backend\":\"%s\","
           ...
           protocol,claim.c_str(),backend_names[WH2_K3_ORDINARY_RECOVERY_BACKEND],wh2_k3_data::kRawSha,
...
int Neutral() {
    ...
    printf("PASS 12 neutral cases, 4 actual API routes, independent field/payload/rank and recovery guards (%u checked)\n",cases);
    return 0;
}
