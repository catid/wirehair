...
#if !defined(WH2_K5_PUBLIC_RECOVERY_BACKEND) || defined(WH_COUNT) || defined(WIREHAIR_TESTING) || defined(WIREHAIR_V2_ENABLE_TEST_HOOKS)
#error "Explicit unmodified recovery backend required"
#endif
static_assert(WH2_K5_PUBLIC_RECOVERY_BACKEND>=0 && WH2_K5_PUBLIC_RECOVERY_BACKEND<=2, "backend");
namespace {
using Byte=uint8_t;
using Profile=std::array<Byte,32>;
const unsigned K=5, arms=6, cpu_seconds=120, wall_seconds=150, address_space_mib=384;
const char protocol[]="wirehair.wh2.k5-public-recovery-r0";
const char claim_path[]="/var/tmp/wh2-k5-public-recovery-r0/CLAIM.json";
    ...
    for(unsigned a=0;a<arms;++a) {
        ...
        if(a==1 || a==4) {
            ...
            Check(wirehair_v2_profile_deserialize(r.profile.data(),32,&p)==WirehairV2_Success &&
                  p.profile_id==WIREHAIR_V2_PROFILE_SMALL_K5_2026_09 && p.seed_attempt==0,
                  "installed sealed K5 descriptor");
        }
        if(a==0 || a==3) {
            WirehairV2Profile p={};
            Check(wirehair_v2_profile_deserialize(r.profile.data(),32,&p)==WirehairV2_Success &&
...
gf256_x86_cpu_features Initialize(const Oracle& oracle) {
    ...
    if(WH2_K5_PUBLIC_RECOVERY_BACKEND==1) Check(!f.SSSE3 && !f.AVX2 && !f.GFNI && !f.AVX512,"portable backend");
...
void Header(const std::string& claim,const char* scope,const gf256_x86_cpu_features& f) {
    printf("{\"type\":\"header\",\"protocol\":\"%s\",\"claim\":\"%s\",\"backend\":\"%s\",\"scope\":\"%s\","
           ...
           protocol,claim.c_str(),backends[WH2_K5_PUBLIC_RECOVERY_BACKEND],scope,wh2_k5_data::kRawSha,
...
int Worker(const std::string& claim) {
    ...
    if(WH2_K5_PUBLIC_RECOVERY_BACKEND!=2) Check(!setrlimit(RLIMIT_AS,&memory),"worker memory cap");
...
int TrackedDecoder(const Profile& p,unsigned b,void*& h) {
    const int r=PDecoder(p,b,h); if(h) ++fault_created; return r;
}
void TrackedFree(void* h) { if(h) ++fault_freed; PFree(h); }
int FaultEncode(void* h,uint32_t id,void* out,unsigned b,uint32_t* n) {
    const int r=PEncode(h,id,out,b,n);
...
int FaultFeed(void* h,uint32_t id,const void* in,unsigned n) {
    const int r=PFeed(h,id,in,n); return ++fault_calls==5?-1:r;
}
int FaultRecover(void* h,void* out,unsigned n,uint64_t* written) {
    const int r=PRecover(h,out,n,written); if(++fault_calls==2) ++*written; return r;
}
...
int main(int argc,char** argv) {
    try {
        if(argc==2 && !strcmp(argv[1],"--contract")) {
            printf("{\"K\":%u,\"arms\":%u,\"records\":6273,\"cpu_seconds\":%u,\"wall_seconds\":%u,\"address_space_mib\":%u,\"gf_context_bytes\":%zu,\"asan_shadow_exempt\":%s,\"backend\":\"%s\",\"claim_path\":\"%s\"}\n",
                   K,arms,cpu_seconds,wall_seconds,address_space_mib,sizeof(gf256_ctx),WH2_K5_PUBLIC_RECOVERY_BACKEND==2?"true":"false",
                   backends[WH2_K5_PUBLIC_RECOVERY_BACKEND],claim_path); return 0;
        }
