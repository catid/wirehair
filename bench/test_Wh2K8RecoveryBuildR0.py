...
import Wh2K8RecoveryBuildR0 as B
...
class BuildParsingTest(unittest.TestCase):
    def commands(self, mode):
        ...
        options = ['-O3', '-DNDEBUG', '-std=gnu++11', '-fPIC', '-Wall', '-Wextra', '-Wpedantic', '-Werror']
        if mode == 'scalar':
            options = ['-DANDROID=1', '-DWIREHAIR_SMALL_EXPECT_PORTABLE=1']+options
        if mode == 'asan':
            options = ['-O1', '-g', '-fsanitize=address,undefined', '-fno-omit-frame-pointer', '-g',
                       '-std=gnu++11', '-fPIC', '-Wall', '-Wextra', '-Wpedantic', '-Werror', '-march=native']
        objects, lines = [], []
        for source in B.PRODUCER_SOURCES:
            obj = prefix+source+'.o'
            objects.append(obj)
            argv = ['/usr/bin/c++']+defines+['-I'+str(B.ROOT/'include')]+options
            argv += ['-MD', '-MT', obj, '-MF', obj+'.d', '-o', obj, '-c', str(B.ROOT/source)]
            lines.append(' '.join(map(shlex.quote, argv)))
        archive = [':', '&&', '/usr/bin/cmake', '-E', 'rm', '-f', 'libwirehair.a', '&&',
                   '/usr/bin/ar', 'qc', 'libwirehair.a']+objects+['&&', '/usr/bin/ranlib', 'libwirehair.a', '&&', ':']
        return ('\n'.join(lines+[' '.join(archive)])+'\n').encode()

    def database(self, mode):
        build = B.SMALL/(mode+'-k8')
        ...
        flags += shlex.split(B.ASAN_FLAGS)+['-g'] if mode == 'asan' else ['-O3', '-DNDEBUG']
    ...
    def test_original_eighteen_object_recipes(self):
        with patch.object(B, 'command', side_effect=AssertionError('must not execute recipe')):
            for mode in B.MODES:
                objects = B.library_commands(self.commands(mode), mode)
                self.assertEqual(len(objects), 18)
                self.assertEqual(len(set(o.name for o in objects)), 18)
                self.assertTrue(all(B.PRODUCTION/B.LIB_DIRS[mode] in o.parents for o in objects))

    def test_original_recipes_reject_flags_roster_and_source_drift(self):
        mutations = [lambda r:r.replace(b'-O3', b'-O2', 1),
                     lambda r:r.replace(b'-Werror', b'', 1),
                     lambda r:r.replace(b'WirehairSmall.cpp', b'WirehairSmallK5.cpp', 1),
                     lambda r:r.replace(b'/usr/bin/c++', b'/usr/bin/clang++', 1),
                     lambda r:r.replace(b'-DNDEBUG', b'-DNDEBUG -march=native', 1),
                     lambda r:b'\n'.join(r.splitlines()[:-1]),
                     lambda r:r+r.splitlines()[0]+b'\n']
        for mutation in mutations:
            with self.assertRaises(ValueError):
                B.library_commands(mutation(self.commands('native')), 'native')
    ...
    def test_boundary_rejects_duplicate_selector_and_backend_drift(self):
        ...
        for mutation in (lambda d:d.pop(), lambda d:d.__setitem__(1, copy.deepcopy(d[0])),
                         ...
                         lambda d:d[0].__setitem__('command', d[0]['command'].replace('=8', '=3', 1))):
    ...
    def test_original_ninja_multiplicity_mtime_and_source(self):
        source = B.HERE/'Wh2SmallSerialized.cpp'
        raw = ('a.o: #deps 2, deps mtime 123 (VALID)\n    '+str(source)+'\n    '+str(source)+'\n\n').encode()
        self.assertEqual(B.library_dependencies(raw, 'a.o', source, 123), {source})
        for bad in [raw.replace(b'#deps 2', b'#deps 1'), raw.replace(b'123', b'122'),
                    raw.replace(b'(VALID)', b'(STALE)'), raw[:-1], raw.replace(b'a.o:', b'b.o:')]:
            with self.assertRaises(ValueError): B.library_dependencies(bad, 'a.o', source, 123)
    ...
    def test_historical_header_not_rebound(self):
        ...
        self.assertEqual(B.CORE_OLD_SHA, '00c299e130df9fbe3acbc920e3f7c94dc72dcd0b4e581dd6dd337735c242da40')
        self.assertEqual(B.SOURCE_HEAD, '421626d0883e11560dac1f635a2ccf68710b30e4')
        self.assertEqual(B.CORE_SHA, '3f534e7b2b127860e5497d9af3d090fe97d300346c116b37cbffea9f7b0d6769')
