#!/usr/bin/env python3
"""Frozen retained-cohort K3/WH1/current-WH2 recovery replay; not a fresh sample."""
import argparse
...
U = sibling('_k3_recovery_build_io', 'Wh2K3ProductionCostR0.py')
D = sibling('_k3_recovery_retained_data', 'Wh2K3NativeDataR0.py')
A = U.A
ROOT = U.ROOT
PROTOCOL = 'wirehair.wh2.k3-recovery-controls-r0'
OUTPUT = Path('/var/tmp/wh2-k3-recovery-controls-r0')
...
SOURCES = ('bench/Wh2K3RecoveryControlsR0.cpp', 'bench/Wh2K3RecoveryControlsR0.py',
           'bench/test_Wh2K3RecoveryControlsR0.py', 'bench/Wh2K3ProductionCostR0.py',
...
def summarize(records, expected):
    ...
    for group, name in enumerate(('retained_fresh', 'hard', 'historical_original_width')):
        ...
        curves = [[sum(a['first'] == 0 or a['first'] > 3+oh for r, _ in selected for a in [r['arms'][arm]])
                   for oh in range(5)] for arm in range(3)] if group != 2 else None
        pairs = []
        for control in (0, 2):
            pair = dict(control=('current_wh2', 'k3', 'wh1')[control], fixed=[], introduced=[])
            for oh in range(5):
                fixed, introduced = [], []
                for r, e in selected:
                    if group == 2 and 3+oh > len(e['ids']):
                        continue
                    x, y = r['arms'][control]['first'], r['arms'][1]['first']
        ...
        groups.append(dict(group=name, cases=len(selected), failure_counts_oh0_to_4=curves,
                           unresolved=[sum(r['arms'][a]['first'] == 0 for r, _ in selected) for a in range(3)],
                           paired=pairs))
    cells = []
    ...
    for width in WIDTHS:
        for schedule in schedules:
            ...
            curves = [[sum(r['arms'][a]['first'] == 0 or r['arms'][a]['first'] > 3+oh for r in selected)
                       for oh in range(5)] for a in range(3)]
    ...
    return dict(outcome='PASS', arms=['current_wh2', 'k3', 'wh1'], groups=groups, cells=cells,
                comparative_oh0_win=fresh[1][0] < fresh[0][0] and fresh[1][0] < fresh[2][0],
...
def verify(raw, claim, mode):
    ...
    for row, e in zip(records, expected):
        ...
        A.exact(len(row['arms']), 3, 'three actual APIs')
        width, ids = e['B'], e['ids']
        for arm, r in enumerate(row['arms']):
            ...
            if arm == 0:
                A.exact(profile[:4], b'WHV2', 'current WH2 descriptor')
                A.exact(profile[16:28], struct.pack('<QI', 3*width, width), 'current WH2 dimensions')
            elif arm == 1:
                A.exact(profile, struct.pack('<4sHHQQII', b'WHK3', 1, 32, 0x5748324b33544d31,
                                            3*width, width, 0), 'sealed installed K3 identity')
            else:
...
def build(mode, output):
    ...
    flags = ['-std=c++11', '-Wall', '-Wextra', '-Wpedantic', '-Werror', '-fno-lto', '-fPIC',
             '-DWIREHAIR_STATIC=1', '-DWH2_K3_RECOVERY_BACKEND='+str(MODES.index(mode)),
