#!/usr/bin/env python3
"""Actual installed WHV2 K8 recovery; six matched arms, retained cohort only."""
import argparse
...
U = sibling('_k8_recovery_build_io', 'Wh2K8PublicRecoveryBuildR0.py')
D = sibling('_k8_recovery_retained_data', 'Wh2K8NativeDataR0.py')
A = U.A
ROOT = U.ROOT
PROTOCOL = 'wirehair.wh2.k8-public-recovery-r0'
OUTPUT = Path('/var/tmp/wh2-k8-public-recovery-r0')
...
K = 8
PROTOTYPE = Path('/var/tmp/wh2-k8-serialized-recovery-r0')
PROTOTYPE_COMPLETE_SHA = '90b532d0ae2a58e3725b2baedaf162bef413a35b86201fbd5fa02c08cb909ea2'
PROTOTYPE_RAW_SHA = '7069bc8ec5ec8895d0a554eb4dbe06bff54796713f082eb6742d45f865bd86c7'
...
SOURCES = ('bench/Wh2K8PublicRecoveryR0.cpp',
           'bench/Wh2K8PublicRecoveryR0.py', 'bench/test_Wh2K8PublicRecoveryR0.py',
           'bench/Wh2K8PublicRecoveryBuildR0.py', 'bench/test_Wh2K8PublicRecoveryBuildR0.py',
    ...
    for row,e in zip(records,expected):
        ...
        for arm,r in enumerate(row['arms']):
            ...
            if arm in (0,3):
                ...
                A.exact(profile,struct.pack('<4sHHQQII',b'WHV2',1,32,0x7a9276b85c730ae0,size,width,0),'installed WHV2 K8 identity')
            ...
            for packet_id,rr,packet in zip(ids,observed,r['packets']):
                # WH1 has no serialized descriptor: bind its dimensions too.
                key = (width,tail,r['profile'],packet_id)
                value = (rr,packet)
                A.exact(value,equations.setdefault(key,value),'repeated descriptor/ID equation and payload')
...
def retained_parity(records):
    """Compare honest installed output with authenticated old prototype records.

    Only the two explicitly validated candidate descriptors may differ; never
    rewrite either stream or weaken the independent installed verification.
    """
    complete_raw = A.read_regular(PROTOTYPE/'COMPLETE.json',4*1024**2)
    A.exact(A.sha(complete_raw),PROTOTYPE_COMPLETE_SHA,'original prototype COMPLETE')
    complete = A.decode(complete_raw)
    A.exact(complete['protocol'],'wirehair.wh2.k8-serialized-recovery-r0','original prototype protocol')
    A.exact(complete['outcome'],'PASS','original prototype outcome')
    raw = A.read_regular(PROTOTYPE/'native.raw.jsonl',RAW_CAP)
    A.exact(A.sha(raw),PROTOTYPE_RAW_SHA,'original prototype native bytes')
    member = [p for p in complete['files'] if p['path']==str(PROTOTYPE/'native.raw.jsonl')]
    A.exact(member,[U.pin(PROTOTYPE/'native.raw.jsonl')],'prototype member binding')
    rows = [A.decode(line) for line in raw.splitlines()]
    A.exact(len(rows),6262,'whole prototype stream')
    A.exact(len(records),6260,'whole installed records')
    for actual,old in zip(records,rows[1:-1]):
        A.exact({k:v for k,v in actual.items() if k!='arms'},
                {k:v for k,v in old.items() if k!='arms'},'prototype chronology')
        A.exact(len(actual['arms']),6,'installed parity arm count')
        A.exact(len(old['arms']),6,'prototype parity arm count')
        for arm,(a,b) in enumerate(zip(actual['arms'],old['arms'])):
            if arm in (1,4):
                size = 7*actual['B']+actual['tail']
                A.exact(a['profile'],struct.pack('<4sHHQQII',b'WHV2',1,32,
                        0x7a9276b85c730ae0,size,actual['B'],0).hex(),'actual installed descriptor')
                A.exact(b['profile'],struct.pack('<4sHHQQII',b'WHK8',1,32,
                        0x5748324b38544d31,size,actual['B'],0).hex(),'original prototype descriptor')
                A.exact({k:v for k,v in a.items() if k!='profile'},
                        {k:v for k,v in b.items() if k!='profile'},'all candidate payload/rank/status/ledger parity')
            else:
                A.exact(a,b,'all control descriptor/payload/rank/status/ledger parity')
