"""Retained K4 native fixture validation, not a new recovery campaign."""
import copy
...
SPEC=importlib.util.spec_from_file_location('_k4_native_tests',Path(__file__).with_name('Wh2K4NativeDataR0.py'))
...
class DataTests(unittest.TestCase):
    ...
    def test_projection_counts_and_failures(self):
        ...
        self.assertEqual({k:len(v) for k,v in data.items()},dict(traces=6216,history=38,windows=31,rows=2252))
        self.assertEqual([sum(t['ranks'][oh]!=4 for t in data['traces']) for oh in range(5)],[9,0,0,0,0])
        self.assertEqual(sum(len(p['ids']) for p in data['history']),163)
        self.assertEqual(len(self.lookup),20480);self.assertEqual(M.C.sha(self.lookup),M.LOOKUP_SHA)
        self.assertEqual(self.lookup[:16],bytes(int(i==j) for i in range(4) for j in range(4)))
        self.assertEqual(M.PAIR[0][0][-1]^M.PAIR[1][0][-1],1)
    ...
    def test_every_emitted_value(self):
        ...
        self.assertEqual(values('kWindows[][8]'),[r['ids'] for r in self.report['seams']])
        self.assertEqual(values('Row kRows[]'),[[r['id'],r['row']] for r in self.report['evidence']['unique_rows']])
        body=header.split('kLookup[20480] = {\n',1)[1].split('};',1)[0]
    ...
    def test_no_old_bundle_receipt_or_candidate_import(self):
        ...
        self.assertNotIn('Wh2K4ThueMorseR0.py',Path(M.__file__).read_text())
    ...
    def test_invalid_projection(self):
        mutations=[lambda r:r['fresh'].pop(),lambda r:r['hard'].pop(),lambda r:r['history'].pop(),
            lambda r:r['inputs']['origins'].pop(),lambda r:r['fresh'][0]['ranks'].__setitem__(0,3),
            lambda r:r['fresh'][0]['ranks'].__setitem__(4,3),lambda r:r['fresh'][0]['ranks'].__setitem__(0,False),
            ...
            lambda r:r['history'][0].__setitem__('rank',3),lambda r:r['pair'][0][0].__setitem__(3,4),
            lambda r:r['seams'].pop(),lambda r:r['seams'][0].__setitem__('deficient',[[0,1]]),
            lambda r:r['seams'][0]['ids'].__setitem__(0,1),lambda r:r['evidence']['unique_rows'].pop(),
    ...
    def test_immutable_output_and_lookup(self):
        ...
        with tempfile.TemporaryDirectory(prefix='wh2-k4-data-neutral-') as directory:
