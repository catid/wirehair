#!/usr/bin/env python3
"""All-sample .76 completion-relative full-decoder screen; replay never executes a codec."""
import argparse
...
A = sibling("_tiny_gfni_cost_common", "Wh2AlignedIntermediateCostR0.py")
T = sibling("_tiny_gfni_cost_lookup", "Wh2ThueMorseTinyPayloadCostR0.py")
...
ROOT = A.ROOT
PROTOCOL = "wirehair.wh2.thue-decoder-cost-r0"
OUTPUT = Path("/var/tmp/wh2-thue-decoder-cost-r0")
TARGET = "wh2_thue_decoder_cost_r0"
RAW_CAP, ERR_CAP, TOTAL_CAP = 48 * 1024**2, 65536, 64 * 1024**2
WORKER_WALL, WORKER_CPU, CONTROLLER_WALL, OBSERVER_WALL = 60, 55, 75, 85
WORK_CAP = 45 * 10**9
BATCH = 128
...
CALLBACKS = 12 * 2 * 3 * 2 * 7 * 18
NEW_SOURCES = tuple("bench/" + name for name in (
    "Wh2ThueMorseDecoderCostR0.py", "test_Wh2ThueMorseDecoderCostR0.py",
    "Wh2ThueMorseDecoderCostR0.cpp",
    "Wh2ThueMorseDecoderCostR0/CMakeLists.txt"))
...
def roster():
    """Fixed cohort; q is a readiness-relative delay, not an absolute phase."""
    index = 0
    for rep in range(12):
        for step in range(2):
            order = (rep + step) % 2
...
def statistics(records):
    ...
    require(len(groups) == 84, "all separate width/metric/class/order groups")
    ...
    for (width, metric, comparison, order), values in sorted(groups.items()):
        ...
        if comparison < 4:
            ...
            # The payload-only overlay leaves decoder equations and bodies unchanged.
            # BC/public require no regression; every WH1 comparison a resolved win.
            limit = 0.0 if comparison == 5 else bound
...
def reduce_records(header, records, footer, raw_sha):
    ...
    require(now >= prelude["observation"][5] and t0 == now, "diagnostic actual-time epoch")
    previous = prelude["observation"]
    work_sum = wait_wall = wait_cpu = 0
    timing = [dict(arm=arm, callbacks=0, starts_late_over_5us=0, max_start_lateness_ns=0,
                   actual_start_phase_histogram=[0]*48, work_bracket_counter_deltas=[0]*4)
              for arm in range(4)]
    ...
    for expected, row in zip(roster(), records):
        ...
        require(target == ready + row["q"], "completion-relative target")
        require(now <= ready and previous[5] <= ready < target, "ordered preparation and positive delay")
        ...
        require(ready <= wm0 <= wm1 <= row["observation"][0] and target <= wm1 and
                previous[4] <= wc0 <= wc1 <= row["observation"][1], "wait ordering")
        require(target <= row["observation"][2], "no early start")
        observed = timing[row["arm"]]
        lateness = row["observation"][2] - target
        observed["callbacks"] += 1
        observed["starts_late_over_5us"] += int(lateness > 5000)
        observed["max_start_lateness_ns"] = max(observed["max_start_lateness_ns"], lateness)
        observed["actual_start_phase_histogram"][(row["observation"][2] % 1000000)*48//1000000] += 1
        for index in range(4):
            observed["work_bracket_counter_deltas"][index] += row["observation"][7][index] - row["observation"][6][index]
        work_sum += duration
        wait_wall += wm1-wm0
        wait_cpu += wc1-wc0
    ...
    analysis.update(protocol=PROTOCOL, claim_sha256=header["claim_sha256"], raw_sha256=raw_sha,
                    ...
                    conditioning=dict(policy="completion_relative_delays_all_samples",
                                      equal_actual_phase_claimed=False,
...
def current_receipt(build, deadline):
    ...
    for name in ("WH2_TINY_GFNI_COST_SANITIZERS", "WH2_TINY_GFNI_SANITIZERS", "BUILD_SHARED_LIBS", "BUILD_TESTS", "BUILD_CODEC_V2",
    ...
    support = ("Wh2ThueMorseTinyGfniR0.cpp", "Wh2ThueMorseNativeCodec.cpp",
               "Wh2ThueMorseTinyGfniR0Codec.cpp", "Wh2ThueMorseTinyGfniR0Test.cpp",
               "Wh2ThueMorseTinyGfniR0CodecTest.cpp", "Wh2ThueMorseTinyGfniR0CodecTest.cpp",
               "Wh2ThueMorseMatvecCostR0Bridge.cpp", "Wh2ThueMorseMatvecCostR0Bridge.cpp",
               "Wh2ThueMorseMatvecCostR0Bridge.cpp", "Wh2ThueMorseMatvecCostR0Bridge.cpp",
               "Wh2ThueMorseDecoderCostR0.cpp", "Wh2RdpruTargetIdentityV2.cpp",
               "Wh2PublicBorrowedTargetIdentity.cpp", "Wh2FrozenTrace.cpp", "Wh2ThueMorseTinyGfniCostR0BridgeTest.cpp")
    expected_paths = [str(ROOT / name) for name in public_files] + [str(ROOT / "bench" / name) for name in support]
    expected_paths.append(str(ROOT / "gf256.cpp"))  # Unlinked original GF audit object.
    exact(sorted(item["file"] for item in commands), sorted(expected_paths), "exact translation units")
    arms, runtime_objects = [], []
    for item in commands:
        ...
        aliases = [word for word in words if word.startswith("-Dwh2_thue_matvec_codec_r0")]
        ...
        if Path(item["file"]).name == "Wh2ThueMorseMatvecCostR0Bridge.cpp":
            ...
            exact(aliases, ["-Dwh2_thue_matvec_codec_r0=wh2_thue_tiny_gfni_r0"]
                  if definitions == ["-DWH2_MATVEC_COST_ARM=1"] else [], "candidate-only payload namespace")
            arms.extend(definitions)
        else:
            require(not definitions and not aliases, "bridge definitions cannot leak")
        if Path(item["file"]).name == "Wh2ThueMorseTinyGfniR0.cpp":
            require(words.count("-o") == 1 and words.index("-o") + 1 < len(words), "runtime object output")
            runtime_objects.append(words[words.index("-o") + 1])
    exact(sorted(arms), ["-DWH2_MATVEC_COST_ARM="+str(i) for i in range(4)], "four exact bridges")
    require(len(runtime_objects) == 1, "one qualified runtime translation unit")
    ...
    link_map = read_regular(build / "tiny-gfni-cost.map", 8 * 1024**2).decode()
    require("libwirehair.a(gf256.cpp.o)" not in link_map and
            "LOAD " + runtime_objects[0] + "\n" in link_map and
            "tiny_gfni_original_gf_audit.dir" not in link_map and
            "Wh2ThueMorseMatvecGfniR0.cpp.o" not in link_map, "single tiny-payload GF runtime object")
    ...
    outputs = {build / name for name in (TARGET, "wh2_tiny_gfni_cost_bridge_test", "lookup.bin", "tiny-gfni-cost.map",
        "tiny-gfni-cost-bridge-test.map", "compile_commands.json", "CMakeCache.txt", "build.ninja", "CMakeFiles/rules.ninja",
        "native-inputs.json", "codec-neutral/tiny_gfni_kernel_test", "codec-neutral/tiny_gfni_kernel_test.map",
        "codec-neutral/tiny_gfni_baseline_test", "codec-neutral/tiny_gfni_baseline_test.map",
        "codec-neutral/tiny_gfni_candidate_test", "codec-neutral/tiny_gfni_candidate_test.map")}
