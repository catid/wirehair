#!/usr/bin/env python3
"""Build an installed WHV2 K8 observer; reproduce all actual archive objects first.
...
FIXTURE = Path('/tmp/wh2-k8-serialized.qQHyZqaa/native-k8/Wh2K8NativeData.inc')
...
def build(mode, output):
    ...
    spec = importlib.util.spec_from_file_location('_k8_public_reader_for_build',HERE/'Wh2K8PublicRecoveryR0.py')
    ...
    dependencies = {ROOT/name for name in reader.SOURCES}|imported_files()
    frozen, commands = {}, []
    ...
    dependencies.update((inventory_path,FIXTURE,reader.PROTOTYPE/'COMPLETE.json',
                         reader.PROTOTYPE/'native.raw.jsonl'))
    dependencies.update(reader.D.ROOT.iterdir())
    ...
    A.exact(pin(output/'Wh2K8NativeData.inc')['sha256'],FIXTURE_SHA,'retained original fixture')
    ...
    flags = ['-std=c++11', '-Wall', '-Wextra', '-Wpedantic', '-Werror', '-fno-lto', '-fPIC',
             '-DWIREHAIR_STATIC=1', '-DWH2_K8_PUBLIC_RECOVERY_BACKEND='+str(MODES.index(mode)),
    ...
    objects = []
    for source in (HERE/'Wh2K8PublicRecoveryR0.cpp', HERE/'Wh2FrozenTrace.cpp'):
    ...
    A.exact(A.decode(contract), dict(K=8, arms=6, records=6260, cpu_seconds=180, wall_seconds=210,
        ...
        claim_path='/var/tmp/wh2-k8-public-recovery-r0/CLAIM.json'), 'compiled neutral/resource contract')
