...
import Wh2K5SerializedRecoveryR0 as C
...
def arm_result(width, ids, arm, deficient=False):
    ...
    if arm in (0,3):
        ...
        profile = struct.pack('<4sHHQQII',b'WHK5',1,32,0x5748324b35544d31,5*width,width,0)
...
class RecoveryTest(unittest.TestCase):
    ...
    def test_only_nonproducing_document_can_use_exact_git_provenance(self):
        old = b'qualified historical document'
        path = C.ROOT/'V2_WIRE_PROFILE.md'
        original = dict(path=str(path),bytes=len(old),sha256=C.A.sha(old))
        current = dict(path=str(path),bytes=7,sha256=C.A.sha(b'current'))
        with patch.object(C.U,'command',return_value=old),patch.object(C.U,'pin',return_value=current):
            recovered,proof = C.qualified_document(original)
            self.assertEqual(recovered,old); self.assertEqual(proof['original'],original)
            self.assertEqual(proof['current'],current)
            wrong = dict(original,path=str(C.ROOT/'codec/WirehairSmallCore.h'))
            with self.assertRaisesRegex(ValueError,'only qualified non-producing document'):
                C.qualified_document(wrong)
        with patch.object(C.U,'command',return_value=b'wrong historical bytes'):
            with self.assertRaisesRegex(ValueError,'exact qualified Git document'):
                C.qualified_document(original)
