...
class CostTest(unittest.TestCase):
    ...
    def test_actual_installed_descriptor_not_prototype_translation(self):
        for width,tail in C.SHAPES:
            profile,_=C.candidate_fixture(width,tail)
            self.assertEqual(profile,struct.pack('<4sHHQQII',b'WHV2',1,32,
                0x7a9276b85c730ae0,7*width+tail,width,0).hex())
        header=copy.deepcopy(self.header)
        for arm in (1,4):
            header['fixtures'][0]['arms'][arm]['profile']=struct.pack(
                '<4sHHQQII',b'WHK8',1,32,0x5748324b38544d31,16,2,0).hex()
        old=dict(fixtures=copy.deepcopy(header['fixtures']))
        with self.assertRaisesRegex(ValueError,'sealed K8 descriptor'):
            C.verify_fixtures(header,old)
    ...
    def test_receipt_cannot_drop_pins_or_rebind_executable(self):
        ...
        receipt = dict(protocol=C.PROTOCOL, head='head', executable=executable,
                       environment=dict({k: None for k in C.ENV_KEYS}, **C.SANITIZERS),
                       pins=sorted(files.values(), key=lambda p: p['path']))
        with patch.dict(os.environ, C.SANITIZERS, clear=True), patch.object(C, 'command', return_value=b'head\n'), \
    ...
    def test_old_scientific_namespace_and_work_unchanged(self):
        old=(C.ROOT/'bench/Wh2SmallLifecycleWorkerR0.h').read_text()
        new=(C.ROOT/'bench/Wh2K8PublicCostR0.cpp').read_text()
        for start,end in (('struct Reader {','struct Work {'),
                          ('NOINLINE void RunWork(', 'void Initialize()')):
        ...
        self.assertEqual(C.OUTPUT,Path('/var/tmp/wh2-k8-public-cost-r0'))
        self.assertNotEqual(C.OUTPUT,Path('/var/tmp/wh2-k8-serialized-cost-r0'))
        self.assertNotIn('wh2_small_',new)
        self.assertNotIn('Wh2SmallSerialized.h',new)
