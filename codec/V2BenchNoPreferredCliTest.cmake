...
execute_process(
    COMMAND "${BENCH}" precodefail --raw-attempt0
        ...
        --paired-overhead-stream --N 4 --bb-list 64 --overhead 0
...
if(NOT raw_result EQUAL 0 OR raw_err OR
   ...
   NOT raw_out MATCHES
       ",0,matrix-c-peel-lo32-xor-hi32-v1,2611923443488327891,0,0x243f6a8885a308d3,0xa19c625b,0x243f6a8885a308d3,0xa19c625b,3,12,12,2,1,0,1,27,0x[0-9a-f]+,[0-9a-f]+[\r\n]*$")
    message(FATAL_ERROR
...
if(NOT usage_result EQUAL 1 OR
   ...
   usage_err MATCHES "preferredattempt|preferredtiming")
    message(FATAL_ERROR
        "preferredattempt leaked into no-preferred usage\n"
        "rc=${usage_result}\nstdout=${usage_out}\nstderr=${usage_err}")
endif()
...
if(NOT timing_result EQUAL 1 OR
   NOT timing_err STREQUAL "unknown mode: preferredtiming\n")
    message(FATAL_ERROR
        "preferredtiming leaked into the no-preferred CLI\n"
        "rc=${timing_result}\nstdout=${timing_out}\n"
        "stderr=${timing_err}")
endif()
...
if(NOT selftest_result EQUAL 0 OR selftest_err OR
   NOT selftest_out MATCHES "loss boundary oracle: PASS")
    message(FATAL_ERROR
        "existing no-preferred CLI command changed\n"
        "rc=${selftest_result}\nstdout=${selftest_out}\n"
        "stderr=${selftest_err}")
endif()
