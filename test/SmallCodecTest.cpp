...
struct Api {
    static constexpr uint64_t ProfileId = WIREHAIR_SMALL_K3_PROFILE_ID;
    static constexpr uint32_t MaxBlockBytes = WIREHAIR_SMALL_K3_MAX_BLOCK_BYTES;
    static WirehairSmallStatus ProfileValidate(const void* p,size_t n) { return wirehair_small_profile_validate(p,n); }
    static WirehairSmallCreateResult EncoderCreate(const void* s,uint64_t m,uint32_t b,uint32_t policy,void* p,size_t n)
    { return wirehair_small_encoder_create(s,m,b,policy,p,n); }
    static WirehairSmallCreateResult EncoderCreateProfile(const void* s,const void* p,size_t n,uint32_t policy)
    { return wirehair_small_encoder_create_profile(s,p,n,policy); }
    static WirehairSmallCreateResult DecoderCreate(const void* p,size_t n) { return wirehair_small_decoder_create(p,n); }
    static WirehairSmallStatus Detach(WirehairSmallCodec h) { return wirehair_small_encoder_detach_input(h); }
    static WirehairSmallResult Encode(WirehairSmallCodec h,uint32_t id,void* p,size_t n) { return wirehair_small_encode(h,id,p,n); }
    static WirehairSmallStatus Decode(WirehairSmallCodec h,uint32_t id,const void* p,size_t n) { return wirehair_small_decode(h,id,p,n); }
    static WirehairSmallResult Recover(WirehairSmallCodec h,void* p,size_t n) { return wirehair_small_recover(h,p,n); }
    static void Free(WirehairSmallCodec h) { wirehair_small_free(h); }
};
#if defined(_MSC_VER)
...
using Profile = std::array<Byte, WIREHAIR_SMALL_PROFILE_BYTES>;
...
struct Oracle {
    ...
    Oracle()
    {
        ...
        for (unsigned phase = 0; phase < 2; ++phase) {
            ...
            for (unsigned i = 0; i < K; ++i) powers[phase][0][i * K + K - 1] = three[i] ^ (i == 0 ? phase : 0);
        }
...
void PacketCheck(WirehairSmallCodec encoder, uint32_t id, const std::vector<Byte>& expected)
{
    std::vector<Byte> packet(expected.size() + 2, 0xa5);
    const WirehairSmallResult short_result = Api::Encode(encoder, id, packet.data() + 1, expected.size() - 1);
    Check(short_result.status == WirehairSmall_BufferTooSmall && !short_result.bytes_written &&
        short_result.bytes_required == expected.size() &&
        std::all_of(packet.begin(), packet.end(), [](Byte b) { return b == 0xa5; }), "short encode no-write");
    Start(0);
    const WirehairSmallResult result = Api::Encode(encoder, id, packet.data() + 1, expected.size());
    const size_t count = Stop();
    Check(!count && result.status == WirehairSmall_Success && result.bytes_required == expected.size() &&
...
void Lifecycle(const Oracle& oracle, uint32_t block, uint32_t tail, uint32_t policy)
{
    ...
    Profile profile;
    profile.fill(0xa5);
    const WirehairSmallCreateResult encoder = Api::EncoderCreate(source.data(), source.size(), block,
        policy, profile.data(), profile.size());
    Check(encoder.status == WirehairSmall_Success && encoder.codec && profile == Descriptor(message.size(), block), "create and exact serialized bytes");
    Check(wirehair_v2_profile_validate(profile.data(), profile.size()) == WirehairV2_InvalidMagic,
          "existing WH2 rejects explicit small-block wire format");

    Check(wirehair_k6_profile_validate(profile.data(), profile.size()) == WirehairK6_UnsupportedProfile,
          "installed K6 rejects K3 profile");
    if (policy == WirehairSmall_Independent) {
        ...
        Check(alias.status == WirehairSmall_InvalidInput && source == message, "borrowed-source write exclusion");
    }
    for (uint32_t id : ids) PacketCheck(encoder.codec, id, oracle.Packet(message, block, id));
    Check(Api::Detach(encoder.codec) == WirehairSmall_Success, "detach succeeds");
    Start(0);
    const auto detached = Api::Detach(encoder.codec);
    Check(Stop() == 0 && detached == WirehairSmall_Success, "detach idempotent allocation free");
    ...
    Check(decoder.status == WirehairSmall_Success && decoder.codec, "standalone decoder");
    ...
    Check(r.status == WirehairSmall_NeedMore && !r.bytes_written && r.bytes_required == message.size(), "initial NeedMore");
    for (unsigned i = 0; i < K; ++i) {
        ...
        Check(Stop() == 0 && (status == WirehairSmall_NeedMore || status == WirehairSmall_Success) && status == again,
    ...
    for (unsigned i = 0; i < K; ++i) {
        ...
        Check(status == WirehairSmall_NeedMore || status == WirehairSmall_Success, "resume with systematic equations");
    }
    r = Api::Recover(decoder.codec, recovered.data() + 1, message.size() - 1);
    Check(r.status == WirehairSmall_BufferTooSmall && !r.bytes_written &&
    ...
    for (unsigned repeat = 0; repeat < 2; ++repeat) {
        ...
        Check(Stop() == 0 && r.status == WirehairSmall_Success && r.bytes_written == message.size() &&
        ...
        Check(Api::Decode(decoder.codec, K, packet.data(), packet.size()) == WirehairSmall_Success, "feed after solved basis");
    }
...
void Malformed()
{
    ...
    auto reject = [&](const void* p, size_t bytes, WirehairSmallStatus expected) {
        Start(0);
        const auto s = Api::ProfileValidate(p, bytes);
        const auto d = Api::DecoderCreate(p, bytes);
        const auto e = Api::EncoderCreateProfile(source.data(), p, bytes, WirehairSmall_Independent);
    ...
    for (size_t bytes = 0; bytes < 32; ++bytes) reject(good.data(), bytes, WirehairSmall_InvalidInput);
    reject(good.data(), 33, WirehairSmall_InvalidInput);
    reject(nullptr, 32, WirehairSmall_InvalidInput);
    reject(reinterpret_cast<void*>(UINTPTR_MAX - 15), 32, WirehairSmall_InvalidInput);
    {
        Profile other = good;
        other[3] = '6';
        Put(other.data() + 8, WIREHAIR_K6_PROFILE_ID, 8);
        reject(other.data(), other.size(), WirehairSmall_UnsupportedProfile);
    }
    for (unsigned i = 0; i < 32; ++i) if (i < 16 || i >= 28)
        for (unsigned bit = 0; bit < 8; ++bit) {
            Profile p = good; p[i] ^= Byte(1u << bit);
            reject(p.data(), p.size(), WirehairSmall_UnsupportedProfile);
        }
    ...
    for (uint64_t message : {UINT64_C(0), uint64_t((K - 1) * 64), uint64_t(K * 64 + 1), UINT64_MAX}) {
        ...
        reject(p.data(), p.size(), WirehairSmall_InvalidDimensions);
    }
    ...
    }
    {
        ...
        Check(Stop() == 0 && s == WirehairSmall_Success, "maximum slab shape accepted without allocation");
    }
    Profile p;
    p.fill(0xa5);
    for (uint32_t policy : {0u, 3u, UINT32_MAX}) {
        ...
        Check(Stop() == 0 && e.status == WirehairSmall_InvalidInput && !e.codec && p[0] == 0xa5, "invalid policy");
    }
    Start(0);
    const auto short_profile = Api::EncoderCreate(source.data(), source.size(), 64,
        WirehairSmall_Independent, p.data(), p.size() - 1);
    Check(Stop() == 0 && short_profile.status == WirehairSmall_BufferTooSmall && !short_profile.codec &&
    ...
    for (const void* invalid : {static_cast<const void*>(nullptr), reinterpret_cast<const void*>(UINTPTR_MAX - 15)}) {
        Start(0);
        const auto bad_source = Api::EncoderCreate(invalid, source.size(), 64, WirehairSmall_Independent, p.data(), p.size());
        const auto bad_output = Api::EncoderCreate(source.data(), source.size(), 64, WirehairSmall_Independent,
            const_cast<void*>(invalid), p.size());
        Check(Stop() == 0 && bad_source.status == WirehairSmall_InvalidInput && !bad_source.codec &&
            bad_output.status == WirehairSmall_InvalidInput && !bad_output.codec, "null/wrapped create buffers rejected");
    }
    Start(0);
    auto e = Api::EncoderCreate(source.data(), source.size(), 64, WirehairSmall_BorrowedImmutable, source.data(), 32);
    Check(Stop() == 0 && e.status == WirehairSmall_InvalidInput && !e.codec && source == original, "borrowed descriptor alias rejected");
    e = Api::EncoderCreate(source.data(), source.size(), 64, WirehairSmall_Independent, source.data(), 32);
    Check(e.status == WirehairSmall_Success && std::equal(good.begin(), good.end(), source.begin()), "independent descriptor alias staged");
    Check(Api::Decode(e.codec, 0, source.data(), 64) == WirehairSmall_InvalidInput &&
          Api::Recover(e.codec, source.data(), source.size()).status == WirehairSmall_InvalidInput,
          "encoder cannot decode or recover");
    Check(Api::Encode(e.codec, 0, source.data(), SIZE_MAX).status == WirehairSmall_InvalidInput,
          "encoder rejects capacity wrapping");
    PacketCheck(e.codec, 0, std::vector<Byte>(original.begin(), original.begin() + 64));
    Api::Free(e.codec);
}

// Byte-basis source blocks expose each entire coefficient row through the
// installed encoder, without consulting its private packed table.
void LookupCoverage(const Oracle& oracle)
{
    std::array<Byte, K * K> source = {};
    for (unsigned i = 0; i < K; ++i) source[i * K + i] = 1;
    Profile profile;
    const auto encoder = Api::EncoderCreate(source.data(), source.size(), K,
        WirehairSmall_BorrowedImmutable, profile.data(), profile.size());
    Check(encoder.status == WirehairSmall_Success, "lookup coverage encoder");
    unsigned checked = 0;
    const auto check = [&](uint32_t id) {
        const Row expected = oracle.Coefficients(id);
        std::array<Byte, K + 2> output; output.fill(0xa5);
        Start(0);
        const auto r = Api::Encode(encoder.codec, id, output.data() + 1, K);
        Check(Stop() == 0 && r.status == WirehairSmall_Success && r.bytes_written == K &&
            r.bytes_required == K && output.front() == 0xa5 && output.back() == 0xa5 &&
            std::equal(expected.begin(), expected.end(), output.begin() + 1), "lookup selector polynomial oracle");
        ++checked;
    };
    for (uint32_t low = 0; low < 1024; ++low) {
        check(low); check(1024u | low);
    }
    for (uint32_t mid = 0; mid < 128; ++mid) for (uint32_t column = 0; column < K; ++column) {
        check((mid << 10) | column);
        check((1u << 17) | (mid << 10) | column);
        check((mid << 17) | column);
        check((1u << 24) | (mid << 17) | column);
    }
    for (uint32_t high = 0; high < 256; ++high) for (uint32_t column = 0; column < K; ++column)
        check((high << 24) | column);
    for (unsigned phase = 0; phase < 2; ++phase) for (unsigned value = 0; value < 128; ++value) {
        check((phase << 17) | (value << 10) | 513u);
        check((phase << 24) | (value << 17) | (85u << 10) | 341u);
    }
    for (unsigned high = 0; high < 256; ++high)
        check((high << 24) | (65u << 17) | (42u << 10) | 777u);
    Check(checked == 5120, "all K3 selectors, matrix columns and mixed chunks");
    Api::Free(encoder.codec);
}
...
void FailuresAndAliases(const Oracle& oracle)
{
    for (unsigned tail : {1u, 64u}) for (uint32_t policy : {1u, 2u}) {
        ...
        const size_t count = Stop();
        Check(created.status == WirehairSmall_Success && count == 2 + (policy == 1 ? 1u : 0u) + (tail != 64 ? 1u : 0u), "account every encoder allocation");
        for (size_t i = 0; i < count; ++i) {
            ...
            Check(alias.status == WirehairSmall_InvalidInput && std::equal(snapshot.begin(), snapshot.end(), static_cast<Byte*>(pointers[i])), "all private encoder allocations protected");
        }
        ...
        for (size_t failure = 0; failure < count; ++failure) {
            ...
            Check(attempted == failure + 1 && created.status == WirehairSmall_OutOfMemory && !created.codec &&
        ...
        for (size_t failure = 0; failure < count; ++failure) {
            ...
            Check(Stop() == failure + 1 && created.status == WirehairSmall_OutOfMemory && !created.codec &&
                  profile == Descriptor(source.size(), 64), "every serialized encoder OOM atomic");
        }
        created = Api::EncoderCreateProfile(source.data(), profile.data(), profile.size(), policy);
        Check(created.status == WirehairSmall_Success, "serialized encoder reconstruction");
        PacketCheck(created.codec, UINT32_MAX, expected);
        Api::Free(created.codec);
        if (policy == 2) for (size_t failure = 0; failure < (tail == 64 ? 2u : 3u); ++failure) {
            created = Api::EncoderCreateProfile(source.data(), profile.data(), profile.size(), policy);
            Check(created.status == WirehairSmall_Success, "detach fixture");
            Start(failure);
            const auto status = Api::Detach(created.codec);
            Check(Stop() == failure + 1 && status == WirehairSmall_OutOfMemory, "every detach OOM");
            PacketCheck(created.codec, UINT32_MAX, expected);
            Check(Api::Encode(created.codec, 0, source.data(), 64).status == WirehairSmall_InvalidInput, "failed detach retains borrowing");
            Check(Api::Detach(created.codec) == WirehairSmall_Success, "detach retry succeeds");
            Api::Free(created.codec);
        }
    }
    ...
    for (size_t failure = 0; failure < 3; ++failure) {
        ...
        Check(Stop() == failure + 1 && d.status == WirehairSmall_OutOfMemory && !d.codec, "every decoder OOM");
    }
    Start();
    const auto d = Api::DecoderCreate(profile.data(), profile.size());
    Check(Stop() == 3 && d.status == WirehairSmall_Success, "decoder wrapper/core/slab accounting");
    for (unsigned i = 0; i < 3; ++i) {
        std::vector<Byte> snapshot(static_cast<Byte*>(pointers[i]), static_cast<Byte*>(pointers[i]) + sizes[i]);
        Check(Api::Decode(d.codec, 0, pointers[i], 64) == WirehairSmall_InvalidInput &&
            Api::Recover(d.codec, pointers[i], (K * 64 - 1)).status == WirehairSmall_InvalidInput &&
            std::equal(snapshot.begin(), snapshot.end(), static_cast<Byte*>(pointers[i])), "all private decoder allocations protected");
    }
    Check(Api::Detach(d.codec) == WirehairSmall_InvalidInput, "decoder cannot detach");
    Byte packet[64] = {};
    Check(Api::Encode(d.codec, 0, packet, 64).status == WirehairSmall_InvalidInput, "decoder cannot encode");
    Check(Api::Decode(d.codec, 0, packet, 63) == WirehairSmall_InvalidInput, "invalid feed does not poison");
    Check(Api::Decode(d.codec, 0, packet, 64) == WirehairSmall_NeedMore, "rank one");
    packet[0] = 1;
    Check(Api::Decode(d.codec, 0, packet, 64) == WirehairSmall_Conflict, "contradiction detected");
    packet[0] = 0;
    Check(Api::Decode(d.codec, 0, packet, 64) == WirehairSmall_Conflict, "persistent poison");
    std::array<Byte, (K * 64 - 1)> output;
    output.fill(0xa5);
    const auto r = Api::Recover(d.codec, output.data(), output.size());
    Check(r.status == WirehairSmall_Conflict && !r.bytes_written && r.bytes_required == output.size() &&
    ...
    Check(resumed.status == WirehairSmall_Success, "resume fixture");
    for (unsigned id = 0; id < K - 1; ++id)
        Check(Api::Decode(resumed.codec, id, packet, sizeof(packet)) == WirehairSmall_NeedMore, "retain deficient basis");
    Check(Api::Recover(resumed.codec, output.data(), output.size()).status == WirehairSmall_NeedMore &&
        std::all_of(output.begin(), output.end(), [](Byte b) { return b == 0xa5; }), "NeedMore preserves output and basis");
    Check(Api::Decode(resumed.codec, K - 1, packet, 64) == WirehairSmall_InvalidInput &&
          Api::Decode(resumed.codec, K - 1, packet, 63) == WirehairSmall_Success, "exact partial tail and resume");
    const void* wrapped = reinterpret_cast<const void*>(UINTPTR_MAX - 15);
    Check(Api::Decode(resumed.codec, 0, wrapped, 64) == WirehairSmall_InvalidInput &&
          Api::Recover(resumed.codec, output.data(), SIZE_MAX).status == WirehairSmall_InvalidInput,
          "wrapped input/capacity rejected");
    Check(Api::Recover(resumed.codec, output.data(), output.size()).status == WirehairSmall_Success &&
          std::all_of(output.begin(), output.end(), [](Byte b) { return b == 0; }), "resumed zero-message oracle");
    packet[0] = 1;
    Check(Api::Decode(resumed.codec, 0, packet, 64) == WirehairSmall_Conflict, "contradiction after solved basis");
    Api::Free(resumed.codec);
    Api::Free(nullptr);
    Check(Api::Detach(nullptr) == WirehairSmall_InvalidInput &&
        Api::Encode(nullptr, 0, packet, 64).status == WirehairSmall_InvalidInput &&
        Api::Decode(nullptr, 0, packet, 64) == WirehairSmall_InvalidInput &&
        Api::Recover(nullptr, packet, 64).status == WirehairSmall_InvalidInput, "null handles");
}
} // namespace
...
#ifdef WIREHAIR_SMALL_EXPECT_PORTABLE
